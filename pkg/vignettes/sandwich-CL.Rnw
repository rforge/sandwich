...
\begin{itemize}
   ...
   \item Experiment II: How do clustered covariances perform for glm types of responses?
   \item Experiment III: How do clustered covariances perform for beyond glm type of responses?
   \item Experiment IV: How do clustered covariances with HC0 - HC3 bias correction perform for different type of responses?
\end{itemize}
Figure (\ref{fig:sim-01}) shows the results from Experiment I and plots the coverage probabilities for the coefficients $x1$ (\ref{x1}), $x2$ (\ref{x2}) and $x3$ (\ref{x3}). Regressor $x1$ is composed of a random draw at cluster level and a random draw at individual level, with a cluster correlation of 0.25. Regressor $x2$ is a random draw at cluster level, and regressor $x3$ is composed of a random draw at individual level.

Depending on the regressors, the implications of changing the cluster correlation $\rho$ from 0 to 0.9 on different covariance estimators are substantial.
Starting with regressor $x1$

...
\new{
In a recent paper, \cite{hac:Abadie+Athey+Imbens+Wooldridge:2017} provide for a condensed summary of state-of-the-art methods for clustered covariances, and focus on situations in economic experiments where  possibly several cluster dimensions exist. In such cases, the sampling process as well as the assignment mechanism exhibiting a systematic variation with clusters in the sample serve as a justification for using clustered covariances. In random experiments, clustered samples are -- with respect to clustered covariances -- less crucial than clustered assignments to various treatments. However, in case of a fixed effects regression with fixed effects at the cluster level, heterogeneity of the treatment effect is in fact a requirement for clustered covariances to be necessary.
}

\new{
Following \cite{hac:Cameron+Gelbach+Miller:2011}, the focus in this manuscript is on situations where the parameter estimator is not adapted. In accordance with the modular implementation of clustered covariances in \pkg{sandwich}, our attention is limited to adjusting inference only.  
}
...
across clusters follows a fixed pattern \citep{hac:Millo:2014, hac:Johnson:2004}. Autocorrelation
...
of only 10 clusters. Theory requires that each cluster dimension have many clusters

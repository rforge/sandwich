...
%% \fronttitle{Econometric Computing with HC and HAC Covariance Matrix Estimators}
%% \frontauthor{Achim Zeileis}
%% \frontdate{October 2004}
%% \frontnumber{10}
...
estimation that provides reusable components which can be used as modular building blocks
...
estimtors \citep[the \pkg{car} package, see][]{hac:Fox:2002} but which does not allow for as much 
...
below is that functions are first-class objects---i.e., functions can take functions as
...
More details about the \proglang{R} code used are provided in an accompanying 
\proglang{R} source file.
...
asymptotically normal. Their covariance matrix $\Psi$ is usually
...
where $\Phi = n^{-1} X^\top \Omega X$ is essiantlly the covariance
...
The estimator HC0 was introduced in the econometrics literature by \cite{hac:White:1980},
building on earlier work by \cite{hac:Eicker:1963},
...
\end{verbatim}
The first argument \code{lmobj} is a fitted linear model object
as returned by \code{lm}.
...
have a slot \code{cov.unscaled} in addition. Currently, this is available for fitted
\code{"lm"}, \code{"glm"} and \code{"rlm"} objects.} \code{lmobj}---from which
...
lead to a VAR(1) model, but also \code{prewhite = p} with \code{p} $> 1$ is possible.}
...
\begin{itemize}
\item \cite{hac:Newey+West:1987} suggest to use linearly decaying weights
...
spectral kernels, \cite{hac:Newey+West:1994} suggest a different nonparametric
...
\item \cite{hac:Lumley+Heagerty:1999} suggest a different approach for specifying 
...
wants to compute them for use in inferential procedures. Therefore, it is
...
for assessing whether a parameter $\beta_j$ is significantly different from zero. These
...
those which produce the figures in the text, are provided (without output) in an
accompanying \proglang{R} source file.
...
The quadratic regression can now be fit easily using the function
...
@
The three estimators lead to slightly different standard errors, but all
...
in the accompanying \proglang{R} source file.
...
par(mfrow = c(1, 2))
bp <- breakpoints(RealInt ~ 1) #Z# argh, but gives nicer figure placement
...
in the package \pkg{sandwich} in the \proglang{R} system for statstical 
...
are first-class objects are of prime importance. 

\section*{Computational details}

The results in this paper were obtained using \proglang{R}
\Sexpr{paste(R.Version()[6:7], collapse = ".")} with the packages \pkg{sandwich}

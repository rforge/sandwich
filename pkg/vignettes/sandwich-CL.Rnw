...
of the estimators and the basic sandwich covariance are able to improve but not fully
...
correlation for the different response distributions. The methods considered are the different
...
or \emph{uncorrelated}. The model equation is given by
...
link function $h(\cdot)$ depends on the model type. The regressors variables are all drawn from standard
...
The vector of coefficients is fixed to either one of
...
zero-truncated Poisson(\code{zerotrunc}, with log link), andzero-inflated Poisson (\code{zip}, with log link and fixed inflation probability $\pi = 0.3$)
distributions.
...
Driscoll and Kraay panel covariances (\code{PL}),
Beck and Katz panel-corrected covariances (\code{PC}),
and clustered covariances with HC0--HC3 adjustment (\code{CL-0}--\code{CL-3}).
As further references, covariances from a clustered bootstrap (\code{BS}),
a mixed-effects model with random intercept (\code{random}), and
from a GEE with exchangeable correlation structure (\code{gee})
are assessed..
...
Table~\ref{tab:vcov} shows exemplary how the different types of covariances are
calculated for responses from the exponential family. A pooled model (\code{m}),
...
#FIXME# levels(s01$vcov)[c(5,2,6,1,4,7,3)] <- c("CL-0", "PL", "PC", "standard", "basic", "random", "gee")
#FIXME# s01$vcov <- factor(s01$vcov, levels(s01$vcov)[c(5,2,6,1,4,7,3)])
...
The coverage (from 10,000 replications) is plotted on the $y$-axis against the cluster correlation $\rho$ on the $x$-axis.
...
As shown in Appendix~\ref{app:ar1}, the performance improves if an AR(1) correlation
structure is employed in the simulation and the number of observations per
cluster increase.
...
experiments focus on the situation with a single regressor \code{x1}.
...
#FIXME# levels(s02$vcov)[c(5,2,6,1,4,7,3)] <- c("CL-0", "PL", "PC", "standard", "basic", "random", "gee")
#FIXME# s02$vcov <- factor(s02$vcov, levels(s02$vcov)[c(5,2,6,1,4,7,3)])
my.settings[["superpose.line"]]$col <- c("#377eb8", "green","#006400", "#ff7f00", "#f781bf", "#984ea3", "#e41a1c")
my.settings[["superpose.symbol"]]$col <- c("#377eb8", "green","#006400", "#ff7f00", "#f781bf", "#984ea3", "#e41a1c")
my.settings[["superpose.symbol"]]$pch <- c(rep(1,3), rep(2,2), rep(3,2))
...
$y$-axis against the cluster correlation $\rho$ (from a Gaussian copula) on the $x$-axis.
The horizontal reference line indicates the nominal coverage of $0.95$.}
\label{fig:sim-02}  
\end{figure}

Figure~\ref{fig:sim-02} illustrates the results from Experiment~II. The settings
are mostly analogous to Experiment~I with two important differences: (1)~A GLM
framework with Gaussian/binomial/Poisson response distribution is used.
...
\fixme{Marginal vs.\ conditional models.} \\
The only major difference between the Gaussian and binomial/Poisson cases is
the performance of the mixed-effects models with random intercept.
These are also called ``conditional models'', and handle correlation by inserting random effects.
``Marginal models'' offer a distinct way to deal with correlation by not introducing random effects, but adjust the covariance matrix instead.  
The distinction between marginal and conditional models is not so important for the Gaussian case with an identity link. However, for non-linear link functions marginal (or ``population-averaged'') means do not have a closed-form expression, because they require expectation of a non-linear function of the random effects. Thus, for the mixed effects model with random intercept, greater caution needs to be exercised. Further details can be found in \cite{hac:Molenberghs+Kenward+Verbeke+Iddi+Efendi:2013} and \cite{hac:Fitzmaurice:2014}.
...
my.settings[["strip.border"]]$col <- "black"
my.settings[["superpose.line"]]$lwd <- 1
#FIXME# levels(s03$vcov)[c(2,3,1)]  <- c("CL-0", "standard", "basic")
#FIXME# s03$vcov <- factor(s03$vcov, levels(s03$vcov)[c(2,3,1)])
...
Figure~\ref{fig:sim-03} shows the outcome of Experiment~III revealing that
the clustered covariances from \code{vcovCL()} indeed work well for more
general response distributions. Again, the performance of the non-clustered 
...
depend on the extent of the correlation $\rho$ this is likely do to the quality
of the normal approximation in the Wald confidence interval.
...
my.settings[["strip.border"]]$col <- "black"
my.settings[["superpose.line"]]$lwd <- 1
#FIXME# levels(s04$vcov)[c(1:4)] <- c("CL-0", "CL-1", "CL-2", "CL-3")
#FIXME# s04$vcov <- factor(s04$vcov, levels(s04$vcov)[c(1:4)])
...
clustered standard errors with HC0--HC3 types of bias correction.
...
binomial and Poisson response). As found in previous studies \citep[][among
...
that about 50 clusters is often enough for accurate coverage rates.
...
\fixme{Brief summary.} \\
While covariances for cross-section and time series data have already been covered in the \pkg{sandwich} package, corresponding functions for clustered or panel data have now been added in an object-oriented manner.
The novelty lies in the direct applicability of \code{vcovCL()}, \code{vcovPL()}, and \code{vcovPC} to models from many packages, as these functions allow plugging in different model objects from in principle any class.
Previously, such an implementation of clustered and panel covariances did not exist in \proglang{R}.
Exploiting the object orientation, a Monte Carlo simulation shows that clustered covariances can be applied not only to linear models, but also to generalized linear models, and even to more general models, as for example to beta regression, zero-truncated Poisson or zero-inflated Poisson models.
...
\fixme{Explanations for panel simulations.}  \\
Figure~\ref{fig:sim-0607} illustrates the behaviour of  panel-corrected, Driscoll and Kraay, and clustered standard errors.
The number of observation per clusters $n_{g}$ is plotted on the $x$-axis, ranging from $5$ (as in Experiments I--III) to $50$.
When modelling the dependency of a Gaussian response via a copula with an AR(1) correlation structure, the empirical coverage converges towards the desired value of $0.95$ for all regressors the higher the number of observations per cluster. For the uncorrelated regressor \code{x3}, panel-corrected standard errors perform well even for $5$ observations per cluster, but underestimation of standard errors is getting even more pronounced the higher the cluster correlation of a regressor.

Using an exchangeable instead of an AR(1) correlation structure, the coverage decreases for Driscoll and Kraay as well as for panel-corrected standard errors with an increasing number of observations per cluster. The aforementioned standard errors assume that correlation is dying out, which is the case for an AR(1) structure, but not for an exchangeable correlation structure, and can explain part of this behaviour. Nevertheless, clustered standard errors perform well for both correlation structures. 

Additionally, Figure~\ref{fig:sim-08} shows that for binomial and Poisson response distributions a similar picture emerges for an AR(1) structure.
...
my.settings[["strip.border"]]$col <- "black"
my.settings[["superpose.line"]]$lwd <- 1
#FIXME# levels(s0607$vcov)[c(2,1,3)] <- c("CL-0", "PL", "PC")
#FIXME# s0607$vcov <- factor(s0607$vcov, levels(s0607$vcov)[c(2,1,3)])
...
xyplot(coverage ~ nround | factor(par) + factor(copula), groups = ~ factor(vcov),
  ...
  type = "b", xlab = expression("n"[g]), ylab = "Empirical coverage",
  auto.key = list(columns = 2),
  par.strip.text = list(col = "black"), par.settings = my.settings,
  panel = panel.xyref)
@ 
...
my.settings[["strip.border"]]$col <- "black"
my.settings[["superpose.line"]]$lwd <- 1
#FIXME# levels(s08$vcov)[c(2,1,3)] <- c("CL-0", "PL", "PC")
#FIXME# s08$vcov <- factor(s08$vcov, levels(s08$vcov)[c(2,1,3)])
my.settings[["superpose.line"]]$col <- c("#377eb8","green", "#006400")
my.settings[["superpose.symbol"]]$col <- c("#377eb8","green", "#006400")

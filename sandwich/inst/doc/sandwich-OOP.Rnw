...
modeling for performing inference that is robust to certain types of misspecification.
...
\pkg{sandwich} is discussed, enabling the computation of sandwich estimators in general
...
and maximum likelihood). Inference for these models, is typically based on a
...
\pkg{sandwich} for the \proglang{R} system for statistical computing \citep{hac:R:2006}
...
covariance matrices in Section~\ref{sec:vcov}. Section~\ref{sec:illustations} gives
...
it is less commonly applied in other models regression models such as GLMs.
...
is required translating the conceptual into computational tools. \proglang{R} comes with
...
are currently available in \proglang{R} packages, e.g., HC and HAC estimators for linear models in
previous versions of \pkg{sandwich}. Therefore, we aim at providing a tool kit
...
functions framework and re-using the computation infrastructure that is already available
...
simply re-use the information provided in the fitted models (or their summaries) and do not
perform any real computations, e.g., for ``\code{lm}'' objects:
\begin{Schunk}
\begin{Sinput}
...
$\psi(y_i, x_i, \hat \theta)$ are more often than not readily implemented in \proglang{R}. Hence,
...
\subsubsection{Outer products estimators}
...
Various function $\omega(\cdot)$ have been suggested that derive estimates of
the variances from the observed working residuals
$(r(y_1, x_1^\top \hat \theta), \dots, r(y_n, x_n^\top \hat \theta))^\top$ and possibly on
...
standard errors of tobit model (for the number of affairs) and a probit model
(for infidelity as a binary variable). In \proglang{R}, these models can be
fitted using \code{survreg()} from the \pkg{survival} package (FIXME: citation)
...
\code{vcov()} and compare them to the HC standard errors provided by
\code{sandwich()}.

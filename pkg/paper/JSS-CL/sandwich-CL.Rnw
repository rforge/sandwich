...
\usepackage{amsmath,amssymb,bm,thumbpdf,lmodern,ulem}
...
\sout{To reflect that the data are now panel data with time ordering within each cluster/group/id we change our notation
to an index $(i, t)$ for $i = 1, \dots, n_t$ observations at time $t = 1, \dots, T$ (with $n = n_1 + \dots + n_T$).}

\new{To reflect that the data are now panel data with a natural time ordering within each entity $g$ (without having a natural ordering, as for example individuals, countries, firms, \ldots) labelled \textit{groups} according to \cite{hac:Millo:2014}, we change our notation
to an index $(g, t)$ for $g = 1, \dots, G$ observations sampled at $t = 1, \ldots, n_g$ points in \textit{time} (with $n = n_1 + \dots + n_G$).}
\sout{Note that compared to the notation from the clustered case above,
...
\begin{equation} \label{eq:newey2}
  \hat M_\mathrm{PL}^{NW} \quad = \quad \frac{1}{n} \sum_{\ell = 0}^{L}\sum_{g,t = 1}^{n} w_{\ell} \, \psi(y_{gt}, x_{gt}, \hat \theta) \psi(y_{gt-\ell}, x_{gt-\ell}, \hat \theta)^\top.
\end{equation}  
}
\cite{hac:Newey+West:1987} employ a Bartlett kernel for obtaining the weights as  
\sout{$w_{|i-j|} = 1 - \frac{|i-j|}{L + 1}$ at lag $\ell = |i-j|$ up to lag $L$} \new{$w_{\ell} = 1 - \frac{\ell}{L + 1}$ at lag $\ell$ up to lag $L$}.
As \cite{hac:Petersen:2009} noticed, the maximal lag length $L$ in a panel data set is \sout{$n_t - 1$} \new{$n_g - 1$,
i.e., the maximum number of \textit{time} periods per \textit{group}} minus one.
...
is now done across \new{\textit{groups} $g$} within each time period $t$. This yields a panel

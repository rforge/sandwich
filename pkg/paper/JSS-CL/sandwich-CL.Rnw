...
strategies is popular for dealing with clustered dependencies in regression models.
...
(QML) estimator or, in GEE jargon, as the independence working model -- but adjusts
...
A further generalization allowing for cluster correlations and heteroscedasticity is provided by the independence working model in GEE \citep{hac:Liang+Zeger:1986}, naturally leading to clustered covariances.
In econometrics, \cite{hac:Arellano:1987} was the first who proposed clustered covariances to fixed effects estimator in panel models.
}

\new{
A comprehensive literature review can be found for instance in \cite{hac:Cameron+Gelbach+Miller:2011} and \cite{hac:Cameron+Miller:2015}. \cite{hac:Cameron+Miller:2015} established a ``model-based perspective'' on clustering with the key assumption that errors are uncorrelated across, but may be correlated for individuals within the same cluster.
In a recent paper, \cite{hac:Abadie+Athey+Imbens+Wooldridge:2017} provide for a condensed summary of state-of-the-art methods for clustered covariances, and see clustering as either a sampling or an experimental design issue. The authors stress that the model-based view of \cite{hac:Cameron+Miller:2015} makes reasoning in favor of one cluster dimension against another difficult. Additionally, it does not explain why there is typically no cluster adjustment in randomized experiments, and why clustering can even be useful in the presence of fixed effect (for a cluster fixed effects regression, parameter heterogeneity is required for the cluster adjustments to be indispensable) \citep{hac:Abadie+Athey+Imbens+Wooldridge:2017}.
Furthermore, while \cite{hac:Cameron+Miller:2015} observe a consensus ``to be conservative and avoid bias and to use bigger and more aggregate clusters when possible, up to and including the point at which there is concern about having too few clusters'' \citep[][p.~333]{hac:Cameron+Miller:2015}, \cite{hac:Abadie+Athey+Imbens+Wooldridge:2017} take the view that ``there is harm in clustering at too aggregate levels `` \citep[][p.~1]{hac:Abadie+Athey+Imbens+Wooldridge:2017}. According to \cite{hac:Abadie+Athey+Imbens+Wooldridge:2017}, cluster adjustments are conservative unless there is no disperson of sample estimates across units (parameter homogeneity), only few clusters are observed from a large population of clusters, or a fading fraction of observations per cluster is sampled. 
...
\new{
Covariance estimators in \pkg{car} as well as in \pkg{sandwich} are constructed to insert the resulting covariance matrix estimate in different Wald-type inference functions, as \code{confint()} from \pkg{stats} package \citep{hac:R:2017}, \code{coeftest()} and \code{waldtest()} from \pkg{lmtest} package \citep{hac:Zeileis+Hothorn:2002}, \code{linerHypothesis()} and \code{deltaMethod()} from \pkg{car} package \citep{hac:Fox+Weisberg:2011}.
}
...
\citep{hac:Hoechle:2007}. While there is wide variety of clustered covariances
...
\left( \begin{array}{c} \new{\displaystyle \sum_{i = 1}^{n_{1}}\psi(y_{i,1}, x_{i,1}, \hat \theta)^\top}\\
           \vdots \\
	  \new{\displaystyle \sum_{i = 1}^{n_{G}}\psi(y_{i,G}, x_{i,G}, \hat \theta)^\top}
           \end{array} \right)
=
\left( \begin{array}{c} \new{\displaystyle \vphantom{\sum_{i = 1}^{n_{1}}} \psi(y_{1,1}, x_{1,1}, \hat \theta)^\top + \dots + \psi(y_{n_1, 1}, x_{n_1, 1}, \hat \theta)^\top}\\
           \vdots \\
	  \new{\displaystyle \vphantom{\sum_{i = 1}^{n_{G}}} \psi(y_{1,G}, x_{1,G}, \hat \theta)^\top + \dots + \psi(y_{n_G, G}, x_{n_G, G}, \hat \theta)^\top}
           \end{array} \right).
\end{eqnarray}  
...
as well as within clusters, \new{assuming that in addition to the number of
observations $n$ the number of clusters $G$ also approaches infinity}\footnote{
\new{An alternative and less frequently-used asymptotic framework to this popular
``large $G$'' approach is a ``fixed $G$'' setup, where the number of clusters is
assumed to be fixed \citep[see][]{hac:Bester+Conley+Hansen:2011,hac:Hansen+Lee:2017}.
However, this also requires non-normal inference and hence cannot be combined with
standard tests in the same modular way as implemented in \pkg{sandwich}. Thus, it
...
are subtracted. 
\new{Particularty when each observation is its own cluster, \cite{hac:Petersen:2009}, \cite{hac:Thompson:2011} and \cite{hac:Ma:2014}
...
or \emph{uncorrelated}. \new{The cluster correlation for the regressors is controlled by parameter $\rho_x$. Note that the previously introduced cluster correlation $\rho$ of the response distributions is determined by yet another parameter $\rho$.} The model equation is given by

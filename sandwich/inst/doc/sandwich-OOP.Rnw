...
%\VignetteDepends{sandwich,zoo,Ecdat}
...
$\theta$ can be estimated using an M-estimator $\hat \theta$ implicitely defined as
  ...
  \end{equation}
This includes (but is not limited to) cases where the estimating function $\psi(\cdot)$ is
...
This brief illustration should merely show that for the \proglang{R} user, the
...
Re-using existing building blocks, all an \proglang{R} developer has to provide for adapting

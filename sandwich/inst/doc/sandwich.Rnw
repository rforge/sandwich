...
\citep[among others]{hac:White:1980,hac:MacKinnon+White:1985,hac:Newey+West:1987,hac:Andrews:1991}
...
econometric computing with HC and HAC estimators?
...
focus on lm() but conceptually extensible.
usually: \code{lm} + \code{vcov} (and \code{summary})\\
now: \code{lm} + \code{vcovHC} or \code{vcovHAC}

functions are first-level objects -- used repeatedly for flexible 
computations (\code{weights}, \code{omega})

Here, linear model, but due to OOP easily extensible. Already applicable
to glm and rlm.
...
fix notation plus kurzer literatur review um daraus die Fkt zu motivieren

ein paar Sachen in \cite{hac:Fox:2002} (aber nicht so flexibel,
extensible), sonst nix
...
\code{lmobj}.\footnote{If \code{weights} is a vector shorter than $n$, it is assumed that
...
\code{as.numeric(prewhite)}, hence both \code{prewhite = 1} and \code{prewhite = TRUE}
...
\end{equation}
where $z = 6 \pi/5 \cdot \ell/B$. The formulas for the remaining kernels can be
...
two strategies suggested in the literature are readily available in \pkg{sandwich}:
First, the weighting scheme
...
and quadratic model---the latter being obviously heavilyy influenced by a single outlier: 
...
interest. The corresponding transformed variable \code{RealInv}, \code{RealGNP}
...
uses the estimator of \cite{hac:Newey+West:1987} with lag $L = 4$ for this purpose
which is here passed as a matrix (as opposed to a function)
to \code{coeftest}.
<<hac-test1>>=
...
\citep{hac:Ploberger+Kraemer:1992} with the same HAC estimator.

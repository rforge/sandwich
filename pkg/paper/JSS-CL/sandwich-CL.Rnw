...
inference functions, as \code{confint()} from \pkg{stats} \citep{hac:R:2017},
...
\new{
To reflect that the data are now panel data with a natural time ordering within each entity \emph{group}~$g$
(without a natural ordering, as for example individuals, countries, or firms, \citealp{hac:Millo:2017}), we change our notation
to an index $(g, t)$ for $g = 1, \dots, G$ \emph{groups} sampled at $t = 1, \ldots, n_g$ points in \emph{time} (with $n = n_1 + \dots + n_G$).
}%
\new{
Clustered covariances for paned data account for the average within-cluster covariance, where a cluster or group is any set of
observations with can be identified by a variable to ``cluster on''.
}%
...
A panel Newey-West estimator can be obtained by setting the cross-sectional as well as the cross-serial correlation to zero \citep{hac:Millo:2017}. The meat is composed of
\new{
...
with both variables: the cluster/group/id and the time/ordering variable,
respectively.
\new{
(The formula specification requires again that \code{expand.model.frame(x, cluster)}
...
well as a dedicated method for \code{lm} objects with more bootstrapping options. Both
methods take the arguments
%
\begin{verbatim}
  vcovBS(x, cluster = NULL, R = 250, ..., use = "pairwise.complete.obs")
\end{verbatim}
%
where \code{cluster} is again a cluster specification as in \code{vcovCL()},
\code{R} is the number of bootstrap replications, and \code{use} is passed on
to the base \code{cov()} function.

The default method samples clusters (where each observation is its own cluster by default),
i.e., case-based ``xy'' resampling \citep[see e.g.,][]{hac:Davison+Hinkley:1997}.
...
listed above, it sends \code{...} to the \code{update()}, and it provides an argument
\code{start = FALSE}. If set to \code{TRUE}, the latter leads essentially to
\code{update(x, start = coef(x), subset = ...)}, i.e., necessitates to support a \code{start}
argument in the model's fitting function in addition to the \code{subset} argument.
If available, this may reduce the computational burden of re-fitting the model.

The method \code{lm} method provides additional bootstrapping \code{type}s and computes the
...
case-based resampling (\code{type = "xy"}). Alternative \code{type} specifications are
%
\begin{itemize}
  \item
...
Finally, the \code{lm} method has some further arguments, namely \code{multi0 = TRUE}
and \code{fix = FALSE} as in \code{vcovCL()} as well as \code{cl = NULL} and \code{parallel = "no"}
that are passed on to \code{boot()} \citep{hac:Canty+Ripley:2017} for parallized bootstrapping.
...
\new{
$\beta = (\beta_{0}, \beta_{1}, \beta_{2}, \beta_{3})^\top$
}%

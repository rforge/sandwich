...
$\hat \approx \mathcal{N}(\theta, n^{-1} S(\theta))$. Moreover, the covariance
...
All in Chapter~\ref{sec:methods} introduced types of clustered covariances differ in the structure of the meat, whereas the bread remains the same.
Thus, in the \pkg{sandwich} package, the \code{bread()} usually does not change, but different meat functions that are based on an \code{estfun()} extractor function are needed, though.
...
Resting upon this, most of the estimators from Chapter~\ref{sec:methods} can be set up in an object-oriented way.
...
HC2 and HC3 types of bias correction are geared towards the linear model, but are also applicable for \code{glm} \citep{hac:Bell+McCaffrey:2002, hac:Kauermann+Carroll:2001}. Nevertheless, a precondition for the HC2 and HC3 bias corrections is the existence of a hat matrix or a weighted version of the hat matrix for GLMs, respectively.
%\fixme{Stress that not just the diagonal, as computed by hatvalues(), is needed but cluster-wise segments of the full
% hat matrix.
% But it is not necessary to extract the full n x n matrix, just the block pertaining to each cluster sequentially.}

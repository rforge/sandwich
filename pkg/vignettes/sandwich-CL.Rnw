...
\subsection[R packages for sandwich covariances]{\proglang{R package for sandwich covariances}}
...
In a nutshell, there is no object-oriented implementation of clustered covariances in R. While the linear regression model is of course an important application case, the same strategies can be employed in more general models, including zero-inflated, censored, or limited responses.

\section[Sandwich covariances in R]{Sandwich covariances in \proglang{R}} \label{sec:implementation}
...
S(\theta)_{id} & = & B(\theta) \cdot M(\theta)_{id} \cdot B(\theta)
\end{eqnarray}
...
\subsection*{One-way clustered standard errors}
...
\subsection*{Reanalysis by \cite{hac:Berger+Stocker+Zeileis:2016}}
...
\end{eqnarray}
with a link function $h$.

\subsection*{Regressor variables}
...
Response distributions examined include Gaussian, Bernoulli (with logit link), Poisson, zero-truncated Poisson, Beta and zero-inflated Poisson (ZIP). With \code{vcovCL()} it is now possible to estimate clustered standard errors for all abovementioned response distributions.

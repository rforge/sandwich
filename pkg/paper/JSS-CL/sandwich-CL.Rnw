...
correlations in the data \new{citep{hac:Abadie+Athey+Imbens+Wooldridge:2017}}.
...
One-way clustered covariances are available in ``Official Stata'' for a wide range of estimators (``ivreg2'', ``xtreg'', ``reg'', among others) by adding the ``cluster'' option. Additionally, ``Official Stata'' supports cluster correction via the ``suest'' command for a system of equations. In the presence of nested multi-level clustering, the ``syv'' suite of commands can be employed \citep{hac:Nichols+Schaffer:2007}. Panel-corrected standard errors are supported by ``xtpcse''. 
The ``Stata Community'' package ``avar'' from \cite{hac:Nichols+Schaffer:2014} allows to construct the meat matrix of various flavors of sandwich covariance estimators, including among others HAC, one- and two-way clustering, Driscoll and Kraay covariances as well as Kiefer covariances \citep{hac:Kiefer:1980}.}
...
\new{
In the statistics literature, the basic sandwich estimator \citep{hac:Eicker:1963,hac:Huber:1967,hac:White:1980} has been extended for clustered units by \cite{hac:Kloek:1981} and \cite{hac:Moulton:1990}, using a correlation structure equivalent to exchangeable correlation in the GEE framework introduced by \cite{hac:Liang+Zeger:1986}.
A relaxation of the assumption imposed by an exchangeable correlation structure of equal correlation whithin clusters has first been incorporated within the GEE framework \citep{hac:Liang+Zeger:1986}, using an independence correlation structure. In econometrics, \cite{hac:Arellano:1987} were the first who proposed clustered covariances to panel data.
}
\\
\cite{hac:Kiefer:1980} robust to within-panel autocorrelation
...
\new{To reflect that the data are now panel data with a natural time ordering within each entity $g$ (without having a natural ordering, as for example individuals, countries, firms, \ldots) labelled \emph{groups} according to \cite{hac:Millo:2014}, we change our notation
...
\begin{equation} \label{eq:newey2}
  \hat M_\mathrm{PL}^{NW} \quad = \quad \frac{1}{n} \sum_{\ell = 0}^{L} w_{\ell} \, \bigg[\sum_{g,t = 1}^{n} \psi(y_{gt}, x_{gt}, \hat \theta) \psi(y_{gt-\ell}, x_{gt-\ell}, \hat \theta)^\top\bigg].
\end{equation}  
}
...
\new{It should be mentioned that the simulation setup constructs a typical case where clustering matters. However, this does not exclude that other situations can occur where clustering is of importance. For example, \cite{hac:Abadie+Athey+Imbens+Wooldridge:2017} show that clustering can matter even in cases with no within cluster correlation of residuals and regressors.
In case of parameter homogeneity clustered covariances are rarely valueable, contrary to situations of parameter heterogeneity. Clustered covariances should be used when heterogeneity in the treatment effect appears. 
In case of a fixed effects regression (with cluster level fixed effects) clustered covariances are necessary only when treatment heterogeneity is present.}

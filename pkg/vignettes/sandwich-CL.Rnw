...
\usepackage{amsmath,amssymb,bm,thumbpdf,lmodern}
...
The only major difference between the Gaussian and binomial/Poisson cases is the
...
confidence intervals do not contain the population-averaged effects as assessed
in Experiment~II.
...
my.settings[["strip.border"]]$col <- "black"
my.settings[["superpose.line"]]$lwd <- 1
#FIXME# levels(s33$vcov)[c(2,3,1)]  <- c("CL-0", "standard", "basic")
#FIXME# s33$vcov <- factor(s33$vcov, levels(s33$vcov)[c(2,3,1)])
...
beyond linear regression. A Monte Carlo study shows that clustered covariances work reasonably well in the models
investigated but some care is needed when applying panel estimators in panel data with ``short'' panels.

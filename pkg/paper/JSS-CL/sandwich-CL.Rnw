...
and \citealp{hac:Freedman:2006}, for a criticial review). \new{This approach leaves
...
\cite{hac:Arellano:1987} was the first who proposed clustered covariances for
...
\begin{eqnarray} \label{eq:biasadj0}
  ...
  \mathrm{HC2:} &  & \new{\frac{G - 1}{G} \cdot (I_{n_{g}} - H_{gg})^{-1}} \\  \label{eq:biasadj3}
  \mathrm{HC3:} &  & \new{\frac{G - 1}{G} \cdot (I_{n_{g}} - H_{gg})^{-2}}, 
\end{eqnarray}  
...
matrix $H$ that pertains to cluster~$g$. \new{In case of a linear model, the hat
...
\begin{equation} \label{eq:fact-adj}
  \tilde r(y_{g}, x_{g}^\top \hat \theta) = \new{\sqrt{\frac{G - 1}{G}} \cdot (I_{n_{g}} - H_{gg})^{-\alpha/2} \cdot r(y_{g}, x_{g}^\top \hat \theta)}
\end{equation}  
\new{with $\alpha = 1$ for HC2 and $\alpha = 2$ for HC3}, before obtaining the adjusted
...
the basic sandwich estimator. \new{The \code{cluster} specification may also be
a \code{formula} if \code{expand.model.frame(x, cluster)} works for the model
object \code{x}.}
...
also be a list \new{or formula} with both variables: the cluster/group/id and the time/ordering variable,
respectively. \new{(The formula specification requires again that \code{expand.model.frame(x, cluster)}
...
\new{
As an alternative to the asymptotic approaches to clustered covariances in \code{vcovCL()},
\code{vcovBS()} provides a bootstrap solution. Currently, a default method is available as
well as a dedicated method for \code{lm} objects with more bootstrapping options.
}

\new{
The default method samples clusters (where each observation is its own cluster by default),
i.e., case-based ``xy'' resampling. For obtaining a covariance matrix estimate it is assumed that an
\code{update()} of the model with the resampled subset can be obtained, the \code{coef()}
extracted, and finally the covariance computed with \code{cov()}.
}

\new{
The method \code{lm} method provides additional bootstrapping variants and computes the
bootstrapped coefficient estimates somewhat more efficiently using \code{lm.fit()} or
\code{qr.coef()} rather than \code{update()}. The default variant, however, is also
case-based resampling (\code{boot\_type = "xy"}). Alternative \code{boot\_type} specifications are
}
\begin{itemize}
  \item \new{\code{"residual"}. The residual cluster bootstrap resamples the residuals (as above,
  ...
  \item \new{\code{"wild"} (or equivalently \code{"wild-rademacher"} or \code{"rademacher"}).
    ...
    probability $0.5$.}
  \item \new{\code{"mammen"} (or \code{"wild-mammen"}). This draws the wild bootstrap factors as
    suggested by \cite{hac:Mammen:1993}).}
  \item \new{\code{"webb"} (or \code{"wild-webb"}). This implements the six-point distribution
    suggested by \cite{hac:Webb:2014}, which may improve inference when the number of clusters is small.}
  \item \new{\code{"norm"} (or \code{"wild-norm"}). The standard normal/Gaussian distribution
    is used for drawing the wild bootstrap factors.}
  \item \new{User-defined function. This should take a single argument \code{n}, the number of
...
separate parameter $\rho_x$. \new{Thus, in the terminology of  \cite{hac:Abadie+Athey+Imbens+Wooldridge:2017},
...
\new{More formally,} the model equation is given by
%

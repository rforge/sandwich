...
need exractor methods \code{estfun.}\textit{foo}\code{()} for these model
classes. In particular, it would be nice to provide such methods for ``\code{survreg}''
and ``\code{coxph}'' objects. As objects from both classes provide some 
...
For ``\code{survreg}'' objects, there is a \code{type = "matrix"} argument
to \code{residuals()} which returns a matrix with partial derivatives of
...
however, the \code{"dg"} component seems to be off by a factor of $2$, whereas
the \code{"ds"} component looks ok.

For ``\code{coxph}'' objects, computing the empirical estimating functions
is taken on by the \code{type = "score"} residuals if I understand the

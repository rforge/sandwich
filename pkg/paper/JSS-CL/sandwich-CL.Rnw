...
One-way clustered covariances are available in Official \proglang{Stata} for a wide range of estimators (\code{ivreg2}, \code{xtreg}, \code{reg}, among others) by adding the \code{cluster} option, and additionally supports cluster correction via the \code{suest} command for a system of equations. In the presence of nested multi-level clustering, the \code{syv} suite of commands can be employed \citep{hac:Nichols+Schaffer:2007}. Panel-corrected standard errors are supported by \code{xtpcse}. 
...
The \proglang{Stata} add-on \pkg{xtscc} provides a further implementation of Driscoll and Kraay covariances \citep{hac:Hoechle:2007}.}
...
clusters $G$ must \new{be large enough} \st{approach infinity}, not just the number of observations $n$

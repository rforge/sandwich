...
Kiefer (1980) investigates fixed effects panel models extended to cases of intertemporal correlations, where the estimated regression coefficients as well as the estimated standard errors are estimated by GLS in a transformed model. As the \texttt{sandwich} package has its focus on a modular adjustment of inference without modifying estimators, we kindly refer to the \texttt{plm} package and its function \texttt{pggls()} that estimates linear panel models by generalized FGLS with (or without) fixed effects.

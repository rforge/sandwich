...
\end{verbatim}
  \begin{itemize}
    ...
    \item \texttt{cadjust}: Switch cluster adjustment $\frac{G}{G-1}$ on and off.
    \item \texttt{multi0}: Should the HC0 estimate be used for the final adjustment in multi-way clustered covariances?
  \end{itemize}
\end{frame}

...
specified so that \code{mclapply()} from the basic \pkg{parallel} package
with the desired number of \code{cores}.
...
The \code{lm} method provides additional bootstrapping \code{type}s and computes the
...
case-based resampling (\code{type = "xy"}) as in the default method. Alternative \code{type} specifications are
%
\begin{itemize}
  \item
...
\end{itemize}  
%
If the basic \pkg{compiler} package is available, \code{cmpfun()} is applied to the bootstrap
refitting function which may slightly speed up computations.
}

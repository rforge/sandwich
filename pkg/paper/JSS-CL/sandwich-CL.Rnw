...
\Abstract{
  ...
  \code{vcovCL()}, \code{vcovPL()}, and \code{vcovPC()}. These are directly
...
if(file.exists("../../vignettes/sim-CL.rda")) {
  load("../../vignettes/sim-CL.rda")
} else {
...
\new{
In the statistics as well as in the econometrics literature is a range of popular strategies available for dealing with clustered dependencies in regression models.
In the following, clustered covariances methods, their historical background as well as a review of existing \proglang{R} and \proglang{Stata} software is discussed.
}
\new{
\subsection{Clustered covariances methods}
}
\new{
...
scores in the estimation \new{\citep{hac:Liang+Zeger:1986}}, also leading naturally to a clustered covariance
(e.g., available in the \pkg{geepack} package for \proglang{R},
\citealp{hac:Halekoh+Hojsgaard+Yan:2002}).
...
\new{
\subsection{History and properties of clustered covariances}
}

\new{
In the statistics literature, the basic sandwich estimator has been introduced first for cross-section data with independent but heteroscedastic observations by \cite{hac:Eicker:1963}, \cite{hac:Huber:1967}, and \cite{hac:White:1980}.
Early references for sandwich estimators accounting for correlated but homoscedastic observations are \cite{hac:Kloek:1981} and \cite{hac:Moulton:1990}, assuming a correlation structure equivalent to exchangeable working correlation introduced by \cite{hac:Liang+Zeger:1986} in the more general framework of generalized estimating equations.
\cite{hac:Kiefer:1980} has implemented yet another clustered covariance estimator for panel data, which is robust to clustering but assumes homoscedasticity \citep[see also][]{hac:Baum+Nichols+Schaffer:2011}.
}

\new{
A further generalization allowing for cluster correlations and heteroscedasticity is provided by the independence working model \citep{hac:Liang+Zeger:1986}, naturally leading to clustered covariances.
In econometrics, \cite{hac:Arellano:1987} was the first who proposed clustered covariances to fixed effects estimator in panel models \citep{hac:Cameron+Miller:2015}.
}

\new{
A very recent paper by \cite{hac:Abadie+Athey+Imbens+Wooldridge:2017} points to the question when one should adjust standard errors for clustering. The authors state that cluster adjustments are conservative unless there is no disperson of sample estimates across units (parameter homogeneity), only few clusters are observed from a large population of clusters, or a fading fraction of observations per cluster is sampled. 
For a cluster fixed effects regression, parameter heterogeneity is required for the cluster adjustments to be indispensable \citep{hac:Abadie+Athey+Imbens+Wooldridge:2017}.
Furthermore, it is recommended that cluster adjustments should not simply be undertakten in each case it makes a difference, but should instead be based on how units in the sample were selected or if the assignment to various treatments is clustered. \cite{hac:Abadie+Athey+Imbens+Wooldridge:2017} state that if the sampling process or the assignment mechanisn exhibits a systematic variation with clusters in the sample, ``clustering will in general be justified''.
}
...
\new{
Official \proglang{Stata} as well as the \proglang{Stata} community provide implementations for several types of clustered covariances.
One-way clustered covariances are available in Official \proglang{Stata} for a wide range of estimators (\code{ivreg2}, \code{xtreg}, \code{reg}, among others) by adding the \code{cluster} option.
Clustered covariances are supported in \proglang{Stata} with the \code{suest} command for seemingly unrelated regression models. In the presence of nested multi-level clustering, the \code{svy} suite of commands can be employed \citep{hac:Nichols+Schaffer:2007}.
The \proglang{Stata} Community package \pkg{avar} \citep{hac:Nichols+Schaffer:2014} allows to construct the meat matrix of various flavors of sandwich covariance estimators, including among others HAC, one- and two-way clustering, Driscoll and Kraay covariances as well as Kiefer covariances \citep{hac:Kiefer:1980}.
The \proglang{Stata} add-on \pkg{xtscc} does also provide an implementation of Driscoll and Kraay covariances \citep{hac:Hoechle:2007}, and \pkg{xtpcse} supports linear regression with panel corrected standard errors.
As pointed out by a referee, although there is wide variety of clustered covariances available in \proglang{Stata}, the selectable options are rather limited. Most notably, it is not possible to choose among different types of finite sample adjustments.}
...
\begin{eqnarray} \label{eq:sandwich}
  ...
  B(\theta) & = & \left( \E\bigg[ - \new{\frac{\partial \psi(y, x, \theta)}{\partial \theta}} \bigg] \right)^{-1} \\  \label{eq:meat}
...
\left( \begin{array}{c} \new{\psi(y_1, x_1, \hat \theta)^\top} \\ \vdots \\ \new{\psi(y_n, x_n, \hat \theta)^\top}
           \end{array} \right).
\end{eqnarray}
...
\begin{equation} \label{eq:Bhat}
  \hat B \quad = \quad \left( \frac{1}{n} \sum_{i = 1}^n - \new{\frac{\partial \psi(y, x, \hat \theta)}{\partial \theta}}   \right)^{-1},
\end{equation}
...
\left( \begin{array}{c} \new{\psi(y_{11}, x_{11}, \hat \theta)^\top + \dots + \psi(y_{n_1 1}, x_{n_1 1}, \hat \theta)^\top}\\
           \vdots \\
	  \new{\psi(y_{1G}, x_{1G}, \hat \theta)^\top + \dots + \psi(y_{n_G G}, x_{n_G G}, \hat \theta)^\top}
           \end{array} \right).
\end{eqnarray}  
...
clusters $G$ must approach infinity\footnote{\new{Asymptotic results can also be achieved with a ``fixed G'' framework -- where the number of clusters is fixed -- as an alternative to the ``large G'' asymptotic, culminating in a non-normal inference theory \citep{hac:Bester+Conley+Hansen:2011,hac:Hansen+Lee:2017}.}}, not just the number of observations $n$
...
matrix $H$ that pertains to cluster~$g$. \new{In case of a linear model, the hat (or projection) matrix is defined as $H = X (X^\top X)^{-1} X^\top$, with model matrix $X$. A generalized hat matrix is available for generalized linear models.}

Thus, it is completely straightforward to add the factors for HC0 and HC1
to $\hat M_\mathrm{CL}$ (Equation~\ref{eq:meatCL}). However, it is more demanding
...
Clustering in two dimensions, for example in $id$ and $time$, gives $D = 2$, such that the clustered covariance matrix is composed of $2^2 - 1 = 3$ one-way clustered covariance matrices that have to be added up or subtracted off, respectively.
...
\new{\cite{hac:Petersen:2009}, \cite{hac:Thompson:2011} and \cite{hac:Ma:2014}
suggest to subtract the standard sandwich estimator instead of $\hat M_{\mathrm{CL}(id \cap time)}$ in Equation (\ref{eq:twoway}) in case there is only a single
...
of \emph{id-month} observations, each cluster is composed of 12 observations \citep{hac:Ma:2014}. In such a setting, the last subtracted matrix of Equation (\ref{eq:twoway}) should be the clustered covariance matrix  $\hat M_{\mathrm{CL}(id \cap year)}$
...
\st{To reflect that the data are now panel data with time ordering within each cluster/group/id we change our notation
to an index $(i, t)$ for $i = 1, \dots, n_t$ observations at time $t = 1, \dots, T$ (with $n = n_1 + \dots + n_T$).}

\new{To reflect that the data are now panel data with a natural time ordering within each entity $g$ labelled \emph{groups} (without a natural ordering, as for example individuals, countries, firms, \ldots) according to \cite{hac:Millo:2014}, we change our notation
to an index $(g, t)$ for $g = 1, \dots, G$ observations sampled at $t = 1, \ldots, n_g$ points in \emph{time} (with $n = n_1 + \dots + n_G$).}
\st{Note that compared to the notation from the clustered case above,
the variable $i$ now denotes the group/id (e.g., firm) which was previously denoted by $g$, whereas time $t$ denotes the cluster variable.}
\new{However, expressions like \emph{groups} and \emph{time} are convenience term and are used for concreteness to make the concepts more easily comprehensible. Clustered covariances for paned data account for the average within-cluster covariance, where a cluster is any set of observations with can be identified by a variable to ``cluster on''.  }
...
\begin{equation} \label{eq:newey2}
  \hat M_\mathrm{PL}^{NW} \quad = \quad \frac{1}{n} \sum_{\ell = 0}^{L} w_{\ell} \, \bigg[\sum_{t = 1}^{n_{g}}\sum_{g = 1}^{G} \psi(y_{gt}, x_{gt}, \hat \theta) \psi(y_{gt-\ell}, x_{gt-\ell}, \hat \theta)^\top\bigg].
\end{equation}  
}
\cite{hac:Newey+West:1987} employ a Bartlett kernel for obtaining the weights as  
\st{$w_{|i-j|} = 1 - \frac{|i-j|}{L + 1}$ at lag $\ell = |i-j|$ up to lag $L$} \new{$w_{\ell} = 1 - \frac{\ell}{L + 1}$ at lag $\ell$ up to lag $L$}.
As \cite{hac:Petersen:2009} noticed, the maximal lag length $L$ in a panel data set is \st{$n_t - 1$} \new{$n_g - 1$,
i.e., the maximum number of \emph{time} periods per \emph{group}} minus one.
...
is now done across \new{\emph{groups} $g$} within each time period $t$. This yields a panel
...
\begin{equation} \label{eq:driscoll}
  \hat M_\mathrm{PL} \quad = \quad \frac{1}{n} \sum_{\ell = 0}^L w_\ell \, \bigg[ \sum_{t = 1}^{n_g} \bigg(
    \sum_{g = 1}^{G} \psi(y_{gt}, x_{gt}, \hat \theta) \bigg) \bigg(
    \sum_{g = 1}^{G} \psi(y_{gt-\ell}, x_{gt-\ell}, \hat \theta) \bigg)^\top \bigg],
\end{equation}  
 } 
The weights \new{$w_{\ell}$} are usually again the Bartlett weights up to lag $L$.
...
index \new{$(g,t)$} is employed for the observation from \new{group} $g = 1, \dots, G$ at time
$t = 1, \dots, n_g$. \new{Here, $n_g$ denotes the last time period observed in cluster $g$, thus allowing for unbalanced data.} In the balanced case (that we focus on below) $n_g = T$ for all groups~$g$
...
Thus, the working residuals are split into \new{$T \times 1$} vectors for each cluster $g$:
...
Section~\ref{ex-aghion} how the same results can now conveniently obtained with the
...
via \code{cluster = InstInnovation$company}.
<<innovation-coeftest>>=
library("sandwich")
library("lmtest")
...
of only 10 cluster. It is required by theory that each cluster dimension has many clusters
...
Panel-corrected covariance a la Beck and Katz are implemented in the package \pkg{pcse} -- providing the
...
linear models. For the linear model, there is a number of simulation studies in
...
correlation \new{$\rho$} for the different response distributions. The methods considered are the different
...
\new{It should be mentioned that the simulation setup constructs a typical case where clustering matters. However, this is not to say that there are other situations not covered here where clustering is important. For example, \cite{hac:Abadie+Athey+Imbens+Wooldridge:2017} show that clustering can be of importance even in cases where no within cluster correlation of residuals and regressors occurs.}
...
or \emph{uncorrelated}. \new{The cluster correlation for the regressors is controlled by parameter $\rho_x$. Note that the previously introduced cluster correlation for the response distributions is determined by yet another parameter $\rho$.} The model equation is given by
\begin{eqnarray} \label{eq:predictor}
h(\mu_{ig}) = \beta_{0} + \beta_{1} \cdot x_{1,ig} + \beta_{2} \cdot x_{2,g} + \beta_{3} \cdot x_{3,ig},
\end{eqnarray}
where $\mu_{ig}$ is the expectation of the response for observation $i$ within cluster $g$ and the
...
\end{eqnarray}
Regressor $x_{1,ig}$ is composed of a linear combination of a random draw at
cluster level ($\mathcal{N}_{g}$) and a random draw at individual level ($\mathcal{N}_{ig}$) while
regressors $x_{2,g}$ and $x_{3,ig}$ are drawn only at cluster and individual level, respectively.
Emphasis is given to the investigation of regressor $x_{1,ig}$ with correlation (default: $\rho_{x} = 0.25$)
which is probably the most common in practice. Furthermore, by considering the extremes $\rho_{x} = 1$ and
$\rho_{x} = 0$ the properties of $x_{1,ig}$ coincide with those of $x_{2,g}$ and $x_{3,ig}$,
respectively.

The vector of coefficients \new{$\beta = (\beta_{0}, \beta_{1}, \beta_{2}, \beta_{3})^\top$} is fixed to either one of
\begin{eqnarray} \label{eq:coefs}
\new{\beta} & = & (0, 0.85, 0.5, 0.7)\new{^\top} \label{beta1} \\ 
\new{\beta} & = & (0, 0.85, 0, 0)\new{^\top} \label{beta2}
\end{eqnarray}
...
(\code{standard}, without considering any heteroscedasticity or clustering/correlations),
basic sandwich covariances (\code{basic}, without clustering),
Driscoll and Kraay panel covariances ({PL}),
Beck and Katz panel-corrected covariances ({PC}),
and clustered covariances with HC0--HC3 adjustment ({CL-0}--{CL-3}).
As further references, covariances from a clustered bootstrap ({BS}),
a mixed-effects model with random intercept ({random}), and
from a GEE with exchangeable correlation structure ({gee})
are assessed.
...
Table~\ref{tab:vcov} shows exemplarily how the different types of covariances are
...
\code{zerotrunc()} (from \pkg{countreg}, \citealp{hac:Zeileis+Kleiber:2017}),
...
my.settings[["superpose.line"]]$col <- c("#377eb8", "green","#006400", "#000080", "darkred", "orange", "black", "grey")
my.settings[["superpose.symbol"]]$col <- c("#377eb8", "green","#006400", "#000080", "darkred", "orange", "black", "grey")
my.settings[["superpose.symbol"]]$pch <- c(rep(1,4), 2, 6, 3, 8)
...
\code{x2}. With increasing cluster correlation $\rho$ the performance of those methods
deteriorate that either do not account for the clustering at all (i.e.,
...
$y$-axis against the cluster correlation \new{of the response distribution} $\rho$ (from a Gaussian copula) on the $x$-axis.
The horizontal reference line indicates the nominal coverage of $0.95$.}
\label{fig:sim-02}  
\end{figure}
...
my.settings[["superpose.line"]]$col <- c("#377eb8", "#000080", "darkred", "orange")
my.settings[["superpose.symbol"]]$col <- c("#377eb8", "#000080", "darkred", "orange")
my.settings[["superpose.symbol"]]$pch <- c(rep(1,2), 2, 6)
xyplot(coverage ~ rho | dist, groups = ~ factor(vcov),
...
$y$-axis for different types of bias adjustment (HC0--HC3) is plotted against the number of
...
that HC2 and HC3 require blockwise components of the full hat matrix and hence
are at the moment only available for \code{lm()} and \code{glm()} fits, see
Section~\ref{sec:vcovcl}.)
...
small samples with less than 250 observations.
...
The authors are grateful to Keith Goldfeld (NYU School of Medicine) for providing insights
and references regarding the differences of conditional and marginal models for clustered
data.
\new{A anonymous referee \ldots}

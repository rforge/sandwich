...
%\VignetteDepends{sandwich,countreg,lattice}
...
@ 
Hurdle models are fitted with the \code{hurdle} function from the \pkg{countreg} package \citep{hac:Zeileis+Kleiber+Jackman:2008}.
<<innovation-model>>=
library("countreg")
h.innov <- hurdle(
...
The packages \pkg{sandwich}, \pkg{countreg} and \pkg{lattice} are

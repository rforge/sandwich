...
not only by \code{lm()}, but also  \code{glm()}, \code{survreg()}, and possibly
...
\cite{hac:Zeileis:2004a}


linear model in \cite{hac:Zeileis:2004a}, now more general
...
conditions, %FIXME% \citep[see e.g.,][]{Z:White:1994}, 
...
%FIXME% further refs \cite{hac:Stefanski+Boos:2002}
...
{
  X <- if(is.matrix(obj$x)) obj$x else model.matrix(obj)
  n <- NROW(X)
...
Models from \cite{hac:Greene:2003} based on \citet{hac:Fair:1978}'s famous
affairs data set, available in \proglang{R} in the \pkg{Ecdat} package
\citep{hac:Croissant:2005}.
...
<<>>=
coeftest(fm_probit)
coeftest(fm_probit, vcov = sandwich(fm_probit, adjust = TRUE))
@

<<>>=
coeftest(fm_tobit)
coeftest(fm_tobit, vcov = sandwich(fm_tobit, adjust = TRUE))
@

mention \code{waldtest()} from \pkg{lmtest} \citep{hac:Zeileis+Hothorn:2002}
and \code{linear.hypothesis()} from \pkg{car} \citep{hac:Fox:2002}

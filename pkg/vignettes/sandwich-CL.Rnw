...
clustered as well as panel data the notation of \citep{hac:Zeileis:2006}
is adopted. Here, the conceptual building blocks from \citep{hac:Zeileis:2006}
...
\left( \begin{array}{c} \psi(y_1, x_1, \hat \theta) \\ \vdots \\ \psi(y_n, x_n, \hat \theta)
           \end{array} \right).
\end{eqnarray}
$\hat B$ extracts the empirical version of the inverse Hessian
...
\begin{eqnarray} \label{eq:meat-op}
  ...
  \end{array} \right) X.
\end{eqnarray}
The outer product estimator in Equation~\ref{eq:meat-op} corresponds to the standard sandwich estimator \citep{hac:Eicker:1963,hac:Huber:1967,hac:White:1980}.
...
\end{equation}  
into empirical working residuals times the regressor vector. This is, however, only possible in situations where the response parameter depends on a single linear predictor. The decomposition of the scores into empirical working residuals and regressor vector requires the model matrix and hence the use of the \code{model.matrix()} method.

The building blocks for the calculation of the sandwich are provided by the \pkg{sandwich} package, where the \code{sandwich} function calculates the sandwich $S(\theta)$ given in Equation~\ref{eq:sandwich} by multiplying a meat (as seen in Equation~\ref{eq:meat}) between two slices of bread (given in Equation~\ref{eq:bread}).
A natural idea for an object-oriented implementation of such estimators is to provide for various functions that compute different estimators for the meat $M(\theta)$ based on an \code{estfun()} extractor function that extracts the empirical estimating functions $\psi(y, x, \theta)$ from a fitted model object (Equation~\ref{eq:estfun}). The meat can be easily exchanged by Equation~\ref{eq:fact}, referring back to \code{estfun()} that extracts Equation~\ref{eq:estfun}.
\code{meat()}, \code{meatHAC()} and \code{meatHC()} compute outer product (Equation~\ref{eq:meat-op}), HAC (Equation~\ref{eq:meat-hac}) and HC estimators (Equation~\ref{eq:meat-hc}) for the meat, respectively, given the existence of an \code{estfun()} method. In addition, the model matrix which is pulled out of the \code{model.matrix()} method is required for \code{meatHC()} (see Equations~\ref{eq:meat-hc} and \ref{eq:fact}).
Additionally, \code{bread()} extracts the bread given in Equation~\ref{eq:bread}, that is the empirical version of the inverse Hessian.
...
For (one-way) clustered covariances, the level of aggregation changes from individuals to clusters/groups, which leads to changes in the meat. 
The double index introduced in the following shows in first place the observation within a cluster, ranging from $1, \ldots , n_g$, and in second place the cluster $g = 1, \ldots, G$ in which the observation is located.
The empirical estimating functions are summed over $g = 1, \ldots, G$ clusters, with $n_1, \ldots, n_G$ observations per cluster, returning a $G \times k$ matrix of empirical estimating functions
...
\end{equation}
The clustered covariance estimator generalizes the standard sandwich estimator in Equation~\ref{eq:Bhat}.
If it is the case that each observation is its own cluster, clustered covariances correspond to standard sandwich covariances.
...
The HC bias correction can additionally to the cluster bias correction be applied to the clustered covariance estimator.
...
$I_{n_{g}}$ is an identity matrix of size $n_{g}$, $H$ is a hat matrix where a block for each cluster $H_{gg}$ is extracted.

HC0 and HC1 types of bias correction can easily be multipied to $\hat M_\mathrm{CL}$ (Equation~\ref{eq:meatCL}), but is more demanding for cluster generalizations of HC2 and HC3 types of bias correction, that have been developed by \cite{hac:Kauermann+Carroll:2001} and \cite{hac:Bell+McCaffrey:2002}.
It should be noted that HC2 and HC3 types of bias correction as seen in Equations~\ref{eq:biasadj3} and \ref{eq:biasadj4} are only available for \code{lm} and \code{glm} objects.
Starting from Equation~\ref{eq:fact}, the groupwise aggregated empirical working residuals $r(y_{g}, x_{g}^\top \hat \theta)$ have to be first adjusted groupwise for cluster generalizations of HC2 and HC3 in
Equations~\ref{eq:biasadj3} and \ref{eq:biasadj4} 
\begin{equation} \label{eq:fact-adj}
  \tilde r(y_{g}, x_{g}^\top \hat \theta) = (I_{n_{g}} - H_{gg})^\alpha \cdot r(y_{g}, x_{g}^\top \hat \theta)
\end{equation}  
with $\alpha = -0.5$ for HC2 type and $\alpha = -1$ for HC3 type of bias correction,
before multiplying the adjusted empirical working residuals with the regressor vector
...
M_{id,time} \quad = \quad M_{id} + M_{time} - M_{id \cap time}.
\end{equation}
Parts of the meat as $M_{id}$ and $M_{time}$ with an odd number of cluster dimensions are added up, whereas those with an even number of cluster dimensions like $M_{id \cap time}$ are substracted off to get the two-way clustered covariance estimator. The same heuristic applies to clustered covariances with more than two dimensions of clusters.

\cite{hac:Petersen:2009}, \cite{hac:Thompson:2011} and \cite{hac:Ma:2014} suggest to substract the standard sandwich estimator  in case that the clusters formed by the intersection of $id$ and $time$ do only contain a single observation\footnote{If one wants to cluster by dimensions \emph{id} and \emph{year} in a sample of \emph{id-month} observations, each cluster is composed of 12 observations \citep{hac:Ma:2014}. In such a setting, the last substracted matrix should be a one-way clustered covariance matrix with clusters formed by the intersections of \emph{id} and \emph{year}.}. 
As \cite{hac:Ma:2014} argues, the standard sandwich estimator does not take into account any bias adjustment, such that the cluster bias correction $\frac{G}{G-1}$ is larger than one. Thus, covariances are underestimated if always the clustered covariance matrix with cluster bias correction $\frac{G}{G-1}$ is substracted as the last substracted matrix instead of standard sandwich covariances.

Various flavours of clustered covariances have been newly implemented in the \pkg{sandwich} package, relying on the same building blocks as in previous package versions. Again, clustered covariances rely on the same structure as seen in Equations~\ref{eq:sandwich}--\ref{eq:meat}. But instead of calling one out of the already implemented meat functions in Equations~\ref{eq:meat-op}--\ref{eq:meat-hc}, \code{meatCL()} is an implementation of the meat as in Equation~\ref{eq:meatCL} and is called via the function \code{vcovCL()} to calculate clustered covariances.
The meat in Equation~\ref{eq:meatCL} is conceptually the same as in Equation~\ref{eq:meat-op}, but with the difference that the score is aggregated at the cluster level as in Equation~\ref{eq:estfun-cl} instead of the unaggregated scores in Equation~\ref{eq:estfun}.
More details about both functions \code{meatCL()} and \code{vcovCL()} are given in the next section.
...
The cluster dimension is now $t$ with $t = 1, \ldots, T$ time periods and $n_t$ observations within a time period.
...
\end{equation}  
where $w_{|i-j|} = 1 - \frac{|i-j|}{L + 1}$ is a distance-decreasing kernel function with a lag $\ell = |i-j|$, $L$ is the maximal lag length. As \cite{hac:Petersen:2009} noticed, the maximal lag length in a panel data set is the maximum number of \emph{time} periods per \emph{id} minus one.
...
Independent of the cross-section dimension, Driscoll and Kraay standard errors are consistent.

Driscoll and Kraay standard errors are a generalization of Newey-West standard errors, applied to the time series of cross-section averages of $\psi(y_{i,t}, x_{i,t}, \hat \theta)$ \citep{hac:Hoechle:2007}.
For the special case that each observation is its own cluster, Driscoll and Kraay standard errors converge to Panel Newey-West standard errors.
...
\begin{equation} \label{eq:driscoll}
  \hat M_\mathrm{PL} \quad = \quad \frac{1}{n} \sum_{t = 1}^T \sum_{i, j = 1}^{n_t} w_{|i-j|} \, \psi(y_{it}, x_{it}, \hat \theta) \psi(y_{jt}, x_{jt}, \hat \theta)^\top,
\end{equation}  
where $i$ denotes the cross section and $t$ the time periods. $w_{|i-j|}$ are again the modified Barttlett weights, and $L$ is the maximal lag length used \citep{hac:Hoechle:2007}. For $\ell = 0$, the meat is equal to one-way clustered covariances clustered by time dimension $t$. 

Equation~\ref{eq:driscoll} does the same thing as Equation~\ref{eq:newey} and Equation~\ref{eq:meat-hac}, respectively, but using aggregated scores from Equation~\ref{eq:estfun-cl} instead of those from Equation~\ref{eq:estfun}.
Driscoll and Kraay as well as Panel Newey-West meat and covariances are implemented in \pkg{sandwich} in functions \code{meatPL()} and \code{vcovPL()}, respectively. They are as well discussed in the Software Section.

\subsection{Panel-corrected standard errors}
Panel-corrected standard errors (hereinafter abbreviated to PCSE) as introduced by \cite{hac:Beck+Katz:1995} fit nicely for time-series-cross-section data characterized by having repeated observations on fixed units with moderate dimensions of time and cross-section \citep{hac:Millo:2014}.
They are robust against panel heteroscedasticity and contemporaneously correlation, with the crucial assumption that contemporaneous correlation accross cluster follows a fixed pattern \citep{hac:Millo:2014, hac:Johnson:2004}. Autocorrelation within a cluster is either not present or has to be removed before PCSE are calculated.
...
To calculate PCSE \citep{hac:Johnson:2004}, the empirical working residuals are separated by cluster dimension $g$, such that they are of form $r(y_1, x_1^\top \hat \theta), r(y_2, x_2^\top \hat \theta), \ldots, r(y_G, x_G^\top \hat \theta)$.
 For balanced data, each empirical working residual is a vector with $t$ elements. Grouped together into a $t \times G$ matrix,
...
\end{equation}
The issue of how to handle unbalanced data sets is discussed in \cite{hac:Bailey+Katz:2011}.

While the Driscoll and Kraay covariance estimator relies on large t-asymptotics and is robust to quite general forms of cross-sectional and temporal dependence and is consistent independently of the cross-sectional dimension,
\cite{hac:Hoechle:2007} argues that for the PCSE estimator the finite sample properties are rather poor if the cross-sectional dimension is large compared to the time dimension.

The newly implemented function \code{vcovPC()} calls \code{meatPC()} to calculate panel-corrected standard errors, and is together with the aforementioned types of clustered covariances for panel as well as cross-section data discussed in detail in the following Section.

\section{Software} \label{sec:software}

All in Section~\ref{sec:methods} introduced types of clustered covariances differ in the structure of the meat, but the \code{bread()} and \code{estfun()} methods remain as previously implemented in \pkg{sandwich}.
Model objects encompassing an \code{estfun} method are one of class \code{clm, coxph, glm, hurdle, lm, mlm, mlogit, nls, polr, rlm, survreg} or \code{zeroinfl}.
Some other packages have their own \code{estfun()} method, as for example \pkg{MASS} \citep{hac:Venables+Ripley:2002}, \pkg{stats} \citep{hac:R:2017}, \pkg{survival} \citep{hac:Therneau:2015}, \pkg{betareg} \citep{hac:Cribari-Neto+Zeileis:2010,hac:Gruen+Kosmidis+Zeileis:2012} and \pkg{crch} \citep{hac:Messner+Mayr+Zeileis:2016}.

Thus, for different types of clustered covariances, different meat functions that are based on an \code{estfun()} extractor function are required.
In case that working residuals are needed, the \code{estfun} is divided through the \code{model.matrix} (see Equation~\ref{eq:fact}).
Resting upon this, most of the estimators from Section~\ref{sec:methods} can be set up in an object-oriented way.
%\fixme{Briefly repeat that the bread() in the sandwich() usually does not change and that `only' different
%meat functions are needed. And the main building block for that is estfun. When working residuals are needed
%these are obtained by dividing the estfun through the model.matrix. And based on this most of the estimators
%from the previous section can be established in an object-oriented way.}

\subsection{Clustered covariances}

One-, two-, and multi-way clustered standard errors with the different types of bias correction introduced in Equations~\ref{eq:biasadj1}--\ref{eq:biasadj4} are implemented with the following arguments in the function
\begin{verbatim}
vcovCL(x, cluster = NULL, type = NULL, sandwich = TRUE, fix = FALSE, ...)
\end{verbatim}
as a part of the \pkg{sandwich} package.
As compared to standard sandwich estimators, the meat for clustered covariances changes and can be separately calculated by the function
\begin{verbatim}
...
The bias correction is composed of two parts, the cluster bias correction defined in Equation~\ref{eq:biasadj0} and the HC bias correction defined in Equations~\ref{eq:biasadj1}--\ref{eq:biasadj4}. Both types of bias correction can be separately switched on and off.
...
HC2 and HC3 types of bias correction are geared towards the linear model, and are at the moment only applicable for \code{lm} and \code{glm} \citep{hac:Bell+McCaffrey:2002, hac:Kauermann+Carroll:2001}.
Hence, this is the only part of \code{vcovCL()} that is not fully object-oriented.
%Nevertheless, a precondition for the HC2 and HC3 bias corrections is the existence of a hat matrix or a %weighted version of the hat matrix for GLMs, respectively.
As already seen in Equation~\ref{eq:fact-adj}, not only is the diagonal as computed by \code{hatvalues()} needed for HC2 and HC3 bias corrections, but cluster-wise segments of the full hat matrix. The blocks pertaining to each cluster are extracted sequentially, thus it is not needed to estract the full $n \times n$ matrix.
...
The \code{kernel} argument is a character specifying the kernel used. Supported are all kernels described in \cite{hac:Andrews:1991}, considering truncated, Bartlett, Parzen, Tykey-Hanning and quadratic spectral kernel \citep[for details, see][]{hac:Zeileis:2004a}. The choice of an appropriate kernel function comes along with setting up a suitable bandwidth parameter. For the special case of a Bartlett kernel, the bandwidth corresponds to the lag.
...
coeftest(p_lm)
@ 
However, the data are clustered by firms (\code{firmid}), with a total of 500 clusters. But ignoring dependency by using ``standard'' standard errors results in an overstate of estimator precision.
...
Four models are fitted: A pooled model (\code{m}), a fixed effects model (\code{m_fe}), a random effects model (\code{m_re}) and a GEE with an exchangeable correlation structure (\code{m_gee}). The fixed effects model includes fixed \code{id} effects, the random effects model is specified as a model with random intercepts. Model fittig for random effects models and GEEs requires the packages \pkg{lme4} and \pkg{geepack}, respectively.
...
\begin{tabular}{llll}
  \hline
  label & model & object & vcov \\ \hline
  \code{CL-0} & \code{lm} & \code{m} & \code{vcovCL(m, cluster = id, type = "HC0")} \\ 
  \code{CL-1} & \code{lm} & \code{m} & \code{vcovCL(m, cluster = id, type = "HC1")} \\
  \code{CL-2} & \code{lm} & \code{m} & \code{vcovCL(m, cluster = id, type = "HC2")} \\ 
  \code{CL-3} & \code{lm} & \code{m} & \code{vcovCL(m, cluster = id, type = "HC3")} \\ 
  \code{PL} & \code{lm} & \code{m} & \code{vcovPL(m, cluster = id, adjust = FALSE)} \\ 
  \code{PC} & \code{lm} & \code{m} & \code{vcovPC(m, cluster = id, order.by = round)} \\ \hline
  \code{standard} &\code{lm} & \code{m} & \code{vcov(m)} \\ 
  \code{basic} & \code{lm} & \code{m} & \code{sandwich(m)} \\ \hline
  \code{fixed} & \code{lm} & \code{m_fe} & \code{vcov(m_fe)[1:3, 1:3]} \\ 
  \code{random} & \code{lmer} & \code{m_re} & \code{vcov(m_re)} \\
  \code{gee} & \code{geeglm} & \code{m_gee}  & \code{m_gee\$geese\$vbeta} \\ \hline
\end{tabular}
...
levels(s01$vcov) <- c("PC", "PL", "gee", "basic", "CL-0", "random", "standard")
my.settings[["superpose.line"]]$col <- c("#006400", "green", "#e41a1c", "#f781bf", "#377eb8", "#984ea3", "#ff7f00")
my.settings[["superpose.symbol"]]$col <- c("#006400", "green", "#e41a1c", "#f781bf", "#377eb8", "#984ea3", "#ff7f00")
...
my.settings[["strip.border"]]$col <- "black"
my.settings[["superpose.line"]]$lwd <- 1
levels(s02$vcov) <- c("PC", "PL", "gee", "basic", "CL-0", "random", "standard")
...
my.settings[["strip.border"]]$col <- "black"
my.settings[["superpose.line"]]$lwd <- 1
levels(s03$vcov) <- c("basic", "CL-0", "standard")
...
my.settings[["strip.border"]]$col <- "black"
my.settings[["superpose.line"]]$lwd <- 1
levels(s04$vcov) <- c("CL-0", "CL-1", "CL-2", "CL-3")
...
my.settings[["strip.border"]]$col <- "black"
my.settings[["superpose.line"]]$lwd <- 1
levels(s05$vcov) <- c("basic", "CL-0", "standard")

...
\Month{August}
\Year{2006}
\Submitdate{2006-05-26}
\Acceptdate{2006-08-15}
...
\Address{
  ...
  URL: \email{http://statmath.wu-wien.ac.at/~zeileis/}\\
}
...
we consider the data from \cite{hac:Deb+Trivedi:1997} on 4406
...
Expenditure Survey, the data is available from the data archive of the
\textit{Journal of Applied Econometrics} at
\url{http://www.econ.queensu.ca/jae/1997-v12.3/deb-trivedi/}. A
``\code{data.frame}'' for usage in \proglang{R} is available along with
this paper as \code{DebTrivedi.rda}. Below, we consider different models
for the number of physician office visits \code{ofp}, explained by regressors
\code{health} status (three-level factor), \code{age} in years divided by 10,
\code{gender}, marital status \code{married}, family income \code{faminc}
(in USD 10,000), and factor \code{privins} indicating private insurance.

First, we use \code{glm()} with \code{family = poisson} to fit a poisson
regression as the simplest model for count data:

<<poisson>>=
load("DebTrivedi.rda")
fm_pois <- glm(ofp ~ health + age + gender + married + faminc + privins, data = DebTrivedi, family = poisson)
coeftest(fm_pois)
@

All coefficients except that of \code{faminc} are highly significant.
However, we are presented a rather different picture when sandwich standard
errors are employed for the partial $z$~tests:

<<poisson-sandwich>>=
coeftest(fm_pois, vcov = sandwich)
@

The explanation for the difference is the overdispersion
in the count variable \code{ofp} which is captured by \code{sandwich()}
but not \code{vcov()} because the \code{poisson} family keeps the dispersion
fixed at 1. Hence, the standard errors provided by \code{vcov()} are misleadingly
small resulting in spuriously significant test statistics.

Of course, sandwich standard errors are not the only way of
dealing with this situation. Other obvious candidates would be to use a
quasi-poisson or a negative binomial model \citep{hac:McCullagh+Nelder:1989}.
...
<<quasipoisson>>=
fm_qpois <- glm(ofp ~ health + age + gender + married + faminc + privins, data = DebTrivedi, family = quasipoisson)
coeftest(fm_qpois)
@
...
<<negbin>>=
fm_nbin <- glm.nb(ofp ~ health + age + gender + married + faminc + privins, data = DebTrivedi)
coeftest(fm_nbin)
@

Here, the estimated parameters (at least those that are significant) are very
similar to those from the poisson regression and the $z$~tests lead to the same
conclusions as in the previous two examples.
...
fm_probit <- glm(I(nbaffairs > 0) ~ age + ym + religious + occupation + rate, 
   data = Fair, family = binomial(link = "probit"))
@
...
\pkg{survival}~\Sexpr{gsub("-", "--", packageDescription("survival")$Version)}

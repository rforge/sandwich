...
estimators \citep[the \pkg{car} package, see][]{hac:Fox:2002} but which does not allow for as much 
...
a similar plot as in the left panel of Figure~\ref{fig:sc}.

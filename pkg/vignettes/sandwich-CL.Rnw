...
$I_{n_{g}}$ is an identity matrix of size $n_{g}$, and $H$ is a block-diagonal hat matrix.
...
\begin{equation} \label{eq:newey}
  \hat M_\mathrm{PL}^{NW} \quad = \quad \frac{1}{n} \sum_{i, j = 1}^n w_{|i-j|} \, \psi(y_{ig}, x_{ig}, \hat \theta) \psi(y_{jg}, x_{jg}, \hat \theta)^\top,
\end{equation}  
where $w_l$ is a distance-decreasing kernel function, and $L$ the maximal lag length. As \cite{hac:Petersen:2009} noticed, the maximal lag length in a panel data set is the maximum number of time periods per id minus one.

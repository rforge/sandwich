...
s03 <- sim(nrep = 10000, nid = 100, nround = 5,
           ...
           type = "copula")
save(s03, file = "sim-02.rda")
}
@

\section{Introduction} \label{sec:intro}

Cluster-correlated data arise when there is a certain degree of similarity between objects of the same group. 
In such datasets with dependence within, but not between observations of a cluster, the assumption of independent and identically distributed errors cannot plausibly sustain.
But ignoring dependency between observations of a cluster by using "classical" standard errors (that is, iid standard errors without heteroscedasticity and without autocorrelation) can result in an overstate of estimator precision and can thus lead to serious mistakes in statistical inference \citep{hac:Moulton:1986, hac:Moulton:1990}.
For valid statistical inference, the correlation between objects of the same group can no longer be ignored.
Thus, clustered covariances are widely used to account for correlated or clustered/grouped data.
Examples from the literature where clustered covariances are used can be found, for example, in the field of economics, political sciences, or other social sciences (references here).
Our paper contributes to the literature particularly in two respects:
...
Second, we perform a simulation study to test the performace of different types of clustered covariances for a variety of response distribution.

The paper is structured as follows: Section~\ref{sec:idea} discusses the general idea of clustered covariances and reviews existing \proglang{R} packages for sandwich as well as clustered covariances.
...
As data described by econometric models typically contain heteroscedasticity and/or autocorrelation of unknown form, it is essential to use covariance matrix estimators for inference that can consistently estimate the covariance of the model parameters.
The idea of a quasi-maximum likelihood approach is to estimate a model by assuming independent data, and then use sandwich covariances to adjust for heteroscedasticity and/or autocorrelation.
Independent but heteroscedastic errors typically occur within a context of cross-section data. Then, heteroscedasticity consistent (HC) covariance estimators can be used to account for the fact that the diagonal elements of the covariance matrix are nonconstant.
For time series data, the independency assumption of the errors often does not hold. For such cases, heteroscedasticity and autocorrelation consistent (HAC) estimators are a suitable option.
...
Clustered covariances are availlable in a couple of R packages, including \pkg{multiwayvcov} \citep{hac:Graham+Arai+Hagstroemer:2016}, \pkg{plm} \citep{hac:Croissant+Millo:2008}, \pkg{geepack} \citep{hac:Halekoh+Hojsgaard+Yan:2002}, \pkg{lfe} \citep{hac:Gaure:2016}, \pkg{clubSandwich} \citep{hac:Pustejovsky:2016}, among others.
...
\code{bread()} extracts the empirical version of the inverse Hessian, and \code{estfun()} extracts empirical estimating functions from a fitted model object. 
...
For clustered covariances, the level of aggregation changes from individuals to clusters, with the result that the meat changes. The empirical estimating functions are first summed groupwise (e.g., by $id$) before the outer product is calculated.
...
One-way clustered covariances can be extended to more than one cluster dimension. In \pkg{sandwich}, \code{vcovCL()} allows for an arbitrary number of cluster dimensions. This so-called multi-way clustering as shown by \cite{hac:Cameron+Gelbach+Miller:2011} comprises clustering on $2^{D} - 1$ dimensional combinations.
Clustering in two dimensions, for example in $id$ and in $time$, gives $D = 2$, such that the clustered covariance matrix is composed of $2^2 - 1$ parts that have to be added up/substracted off.
...
\end{eqnarray}
Clustered covariance matrices with an odd number of cluster dimensions are added, whereas those with an even number of cluster dimensions are substracted off.
...
Compared to classical sandwich estimators, the meat changes for clustered covariances. The function \code{meatCL(x, cluster = NULL, type = NULL, cadjust = TRUE, white = FALSE, \ldots)} allows the calculation of the meat for clustered covariances.
...
The \code{white} argument specifies whether to substract HC0-type (that is, White's) covariance matrix as the last substracted matrix or the covariance matrix formed by the intersection of groups \citep{hac:Ma:2014}.
The \code{sandwich} argument returns either the full sandwich or the meat, and the \code{fix} argument specifies the eigendecomposition of the estimated covariance matrix and convert any negative eigenvalues to zero.
...
CONTINUE HERE
For panel data, \code{vcovPL()} allows for estimation of Driscoll and Kraay type covariances \citep{hac:Driscoll+Kraay:1998} as well as Newey-West type covariances \citep{hac:Newey+West:1987}.
\cite{hac:Driscoll+Kraay:1998} apply a Newey-West type correction to the sequence of cross-sectional averages of the moment conditions \citep{vcov:Hoechle:2007} \par
for large length of the time dimesion (and regardless of the length of the cross-sectional dimension), the \cite{hac:Driscoll+Kraay:1998} standard errors are robust to general forms of cross-sectional and serial correlation \citep{hac:Hoechle:2007} \par
...
\code{vcovPL(x, cluster = NULL, order.by = NULL, kernel = "Bartlett", lag = "max", \ldots)}
the \code{cluster} argument allows to specify a variable indicating the clustering of observations
the \code{order.by} argument allows to specify a variable indicating the aggregation within time periods
the \code{kernel} argument is a character specifying the kernel used \par 
  all kernels described in \cite{hac:Andrews:1991} are supported
the \code{lag} argument indicates the lag length used
the \code{bw} argument allows to specify the bandwidth of the kernel which by default corresponds to $\textrm{lag} + 1$
the \code{sandwich} argument specifies whether the sandwich estimator should be computed, if set to \code{FALSE}, only the meat matrix is returned
the \code{fix} argument allows the covariance matrix be fixed to be positive semi-definite
...
\code{vcovPC(x, cluster = NULL, order.by = NULL, subsample = TRUE, sandwich = TRUE, fix = FALSE, \ldots)}
...
We run a Monte Carlo simulation to investigate the performance of a variety of clustered standard errors for different response distributions.
...
xyplot(coverage ~ rho | par, groups = ~ factor(vcov),
  ...
  panel = panel.xyref)
@ 
\caption{Fixed: $N_{g} = 5$, $G = 100$, $\rho_{x} = 0.25$, $F = \mathcal{N}$.}
  \end{center}
\end{figure}
...
xyplot(coverage ~ rho | dist, groups = ~ factor(vcov),
  ...
  panel = panel.xyref)
@ 
\caption{Fixed: $N_{g} = 5$, $G = 100$, $\rho_{x} = 0.25$, $F = \mathcal{N}$, Poisson, logit.}
  \end{center}
\end{figure}
...
my.settings[["strip.border"]]$col <- "black"
my.settings[["superpose.line"]]$lwd <- 1
levels(s03$vcov) <- c("classical", "HC0", "HC0-cluster")
my.settings[["superpose.line"]]$col <- c("#ff7f00", "#f781bf", "#377eb8")
my.settings[["superpose.symbol"]]$col <- c("#ff7f00", "#f781bf", "#377eb8")
xyplot(coverage ~ rho | dist, groups = ~ factor(vcov),
  ...
  panel = panel.xyref)
@ 
\caption{Fixed: $N_{g} = 5$, $G = 100$, $\rho_{x} = 0.25$, $F =$ zerotrunc. Poisson, ZIP, beta.}
  \end{center}
\end{figure}

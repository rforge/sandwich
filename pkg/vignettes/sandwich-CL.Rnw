...
The \code{white} argument is executed only for two- and multi-way clustered
covariances and specifies whether to substract White's covariance matrix as the
last substracted matrix in Equation~\ref{eq:twoway} or the covariance matrix
formed by the intersection of groups
\citep{hac:Petersen:2009,hac:Thompson:2011,hac:Ma:2014}.
...
\end{verbatim}
compute sandwich covariances for panel data, specificially including
...
\code{sandwich = TRUE}, and \code{fix = TRUE} converts any negative eigenvalues to zero.

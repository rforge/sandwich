...
s01 <- sim(nrep = 10000, nid = 100, nround = 5,
           ...
           vcov = c("classical", "HC0", "HC0-cluster", "random", "gee", "PC", "DK"),
           type = "copula")
...
s02 <- sim(nrep = 10000, nid = 100, nround = 5,
           dist = c("gaussian", "binomial(logit)", "poisson"), rho = seq(0, 0.9, by = 0.1), xrho = 0.25,
...
s03 <- sim(nrep = 10000, nid = 100, nround = 5,
           ...
           coef = c(0, 0.85, 0, 0), formula = response ~ x1,
           vcov = c("classical", "HC0", "HC0-cluster"),
           type = "copula")
...
s04 <- sim(nrep = 10000, nid = c(10, seq(50, 250, by = 50)), nround = 5,
           dist = c("gaussian","poisson", "binomial(logit)"), rho = 0.25, xrho = 0.25,
           coef = c(0, 0.85, 0, 0), formula = response ~ x1,
           vcov = c("HC0-cluster","HC1-cluster","HC2-cluster","HC3-cluster"),
           type = "copula")
...
if(file.exists("sim-06.rda")) load("sim-06.rda") else {
set.seed(6)
...
if(file.exists("sim-07.rda")) load("sim-07.rda") else {
set.seed(7)
...
<<sim-08,echo=FALSE,results=hide>>=
if(file.exists("sim-08.rda")) load("sim-08.rda") else {
set.seed(8)
s08 <- sim(nrep = 10000, nround = c(5, 10, 20, 50), nid = 100,
           dist = c("binomial(logit)", "poisson"), rho = 0.25, xrho = 0.25,
           coef = c(0, 0.85, 0.5, 0.7), formula = response ~ x1 + x2 + x3,
           vcov = c("HC0-cluster", "PC", "DK"), type = "copula-ar1")
save(s08, file = "sim-08.rda")
}
@
...
package. One- and two-way clustered standard errors from \code{vcovCL()} from \pkg{sandwich} are compared to those obtained using \code{cluster.vcov()} from \pkg{multiwayvcov} package. Furthermore, one-way clustered standard errors can also be obtained from packages \pkg{plm} and \pkg{geepack}, and are additionally benchmarked against the results from \pkg{sandwich}. 
Function \code{vcovSCC()} from \pkg{plm} gives Driscoll and Kraay type standard errors, the estimates are compared to \code{vcovPL()} from \pkg{sandwich}. 
Also, panel-corrected standard errors can be estimated by function \code{vcovPC()} from \pkg{pcse} and are benchmarked against \code{vcovPC()} from \pkg{sandwich}.
...
sapply(vc, function(x) sqrt(diag(x)))
@ 
The bias correction in \code{vcovCL()} is set to \code{cadjust = TRUE} and \code{type = "HC1"} by default, which is the same default as in \code{cluster.vcov()}.

Furthermore, there are implementations for clustered covariances in packages \pkg{plm} and \pkg{geepack}.
<<petersen-packages2>>=
library("plm")
library("geepack")
@ 
...
At least for coefficient \code{x}, two-way clustered standard errors with cluster dimensions \code{firm} as well as \code{year} are even higher than one-way clustered standard errors.
...
Panel-corrected standard can be obtained with function \code{vcovPC()} for balanced and unbalanced panels.
For the balanced Petersen data, \code{pcse()} and \code{vcovPC()} deliver the same results.
...
sapply(vc, function(x) sqrt(diag(x)))
@
In case of an unbalanced panel, the \code{subsample} argument allows to choose either a pairwise balanced sample or a balanced subset of the panel.
For illustrative purposes, an unbalanced version of the Petersen data is constructed,
<<petersen-unbalanced1>>=
unbal <- sample(1:length(unique(PetersenCL$firm)), 10)
PeUn <- PetersenCL[-unbal, ]
un_lm <- lm(y ~ x, data = PeUn)
@ 
and again, panel-corrected standard errors from \pkg{sandwich} are equivalent to those from \pkg{pcse}.
<<petersen-unbalanced2>>=
vc <- list(
"sandwichF" = sandwich::vcovPC(un_lm, cluster = PeUn$firm,
order.by = PeUn$year, subsample = FALSE),
"pcseT" = pcse::vcovPC(un_lm, PeUn$firm, PeUn$year, pairwise = TRUE),
"sandwichT" = sandwich::vcovPC(un_lm, cluster = PeUn$firm,
order.by = PeUn$year, subsample = TRUE),
"pcseF" = pcse::vcovPC(un_lm, PeUn$firm, PeUn$year, pairwise = FALSE)
)
sapply(vc, function(x) sqrt(diag(x)))
@ 
...
For a more systematic analysis, a Monte Carlo simulation was carried out to assess the performance of clustered covariances beyond linear and generalized linear models. For the linear model, there are a couple of simulation studies in the literature \citep{hac:Cameron+Gelbach+Miller:2008,hac:Arceneaux+Nickerson:2009,hac:Petersen:2009,hac:Cameron+Gelbach+Miller:2011,hac:Harden:2011,hac:Thompson:2011,hac:Cameron+Miller:2015,hac:Jin:2015}, far less for generalized linear models \citep[see for example][]{hac:Miglioretti+Heagerty:2007} and, up to our knowledge, none for models beyond \code{lm()} and \code{glm()}.

\subsection{Simulation design}
Parameters $\rho$ and $G$ vary systematically, where $\rho$ determines the strength of cluster correlation and varies from $0$ to $0.9$. The number of clusters $G$ ranges from $10$ to $50, 100, 150, 200$ to $250$. Numerous studies \citep{hac:Green+Vavreck:2008,hac:Arceneaux+Nickerson:2009,hac:Harden:2011} confirmed that for the linear regression case, the higher the number of clusters, the less standard errors are biased. Furthermore, only balanced cluster are investigated, with the number of  observations per cluster fixed to $5$.
...
In order to introduce cluster correlation in the response, two options are investigated. The first option is to take the marginal model Equation~\ref{eq:predictor} and introduce correlation via a Gaussian copula. The second option is to add a random effect in Equation~\ref{eq:predictor}. For the linear model with an identity link function $h(\cdot)$, both options amount to the same thing. Though for models other than \code{lm()}, the alternatives are different.
The copula option is favored in the simulation exercises, as for models other than \code{lm()}, including a random effect comes along with including a bias.
...
\end{eqnarray}
Regressor $x_{1,ig}$ in Equation~\ref{x1} is composed of a linear combination of a random draw at cluster level ($\mathcal{N}_{g}$) and a random draw at individual level ($\mathcal{N}_{ig}$). Regressor $x_{2,g}$ in Equation~\ref{x2} is composed of a random draw at cluster level, and regressor $x_{3,ig}$ in Equation~\ref{x3} consists of a random draw at individual level. In most simulations, a single regressor $x_{1,ig}$ is investigated.

In line with \cite{hac:Harden:2011}, this setup is used to introduce correlated (\ref{x1}), clustered (\ref{x2}) and uncorrelated regressors (\ref{x3}). The cluster correlation of $x_{1,ig}$ is controlled by parameter $\rho_{x}$, and set to 0.25 per default. This implies at least some within cluster correlation of regressor $x_{1,ig}$. For $\rho_{x} = 1$, regressors $x_{1,ig}$ and $x_{2,g}$ are equivalent. Furthermore, if $\rho_{x} = 0$, regressor $x_{1,ig}$ corresponds to regressor $x_{3,ig}$.
...
Response distributions examined include Gaussian (\code{gaussian}), binomial with a logit link (\code{binomial(logit)}), Poisson (\code{poisson}), zero-truncated Poisson (\code{zerotrunc}), Beta (\code{beta}) and zero-inflated Poisson (\code{zip}). Again, \code{vcovCL()} allows estimation of clustered covariances for all abovementioned responses (and many more).

\subsubsection{Sandwich covariances}
Covariances being compared to each other include ``standard'' covariances without heteroscedasticity and without autocorrelation (\code{standard}), basic sandwich covariances (\code{basic}), Driscoll and Kraay type covariances (\code{PL}), panel-corrected covariances (\code{PC}) a la \cite{hac:Beck+Katz:1995} and clustered covariances with HC0 to HC3 adjustment (\code{CL-0}, \code{CL-1}, \code{CL-2}, \code{CL-3}). In addition, covariances from a random effects model (\code{random}) and from a GEE with exchangeable correlation structure (\code{gee}) are estimated.

\subsubsection{Outcome measure}
In order to assess the validity of statistical inference, the empirical coverage rate is the outcome measure of interest.
If standard errors are estimated accurately, the empirical coverage rate of the $95$\% confidence interval should be close to 0.95. 
...
The \proglang{R} script \code{sim-CL.R} comprises the simulation code for the data generating process  described above and includes functions \code{dgp()}, \code{fit()} and \code{sim()}.
...
Table~\ref{tab:vcov} shows exemplary how the different types of covariances are calculated for a Gaussian response.
A pooled model (\code{m}),
% a fixed effects model (\code{m_fe}),
a random effects model (\code{m_re}) and a GEE with an exchangeable correlation structure (\code{m_gee}) is fitted. 
...
However, it is not possible to fit all model types for all mentioned response distributions. There are some limitations for random effects models as well as for GEEs.
...
\begin{itemize}
   \item Experiment I: How do clustered covariances perform for different types of regressors for a Gaussian response distribution?
   \item Experiment II: How do clustered covariances perform for glm response distributions?
   \item Experiment III: How do clustered covariances perform for more general model response distributions?
   \item Experiment IV: Which type of HC0--HC3 bias correction performs best?
\end{itemize}

\subsubsection{Experiment I}
Figure~\ref{fig:sim-01} shows the results from Experiment I and plots the coverage probabilities for coefficients \code{x1} (Equation~\ref{x1}), \code{x2} (Equation~\ref{x2}) and \code{x3} (Equation~\ref{x3}) on the y-axis.
On the x-axis the cluster correlation of the response $\rho$ is shown, running from 0 (no correlation) to 0.9 (high correlation).
Depending on the regressors, the implications of running the cluster correlation $\rho$ from 0 to 0.9 on the coverage of different covariance estimators are substantial.

Starting with regressor \code{x1} at $\rho = 0$, all standard errors except the Driscoll and Kraay standard errors perform well and are close to the ``true'' standard error. As the number of observations per cluster is small with only $N_g = 5$, Driscoll and Kraay standard errors are substantially underestimated, even at a $\rho$ of 0. 
In a Monte-Carlo study, \cite{hac:Driscoll+Kraay:1998} put the minimum of $N_g > 20 - 25$.
...
In summary, it holds for the linear model that if a regressor exhibits within cluster correlation even to a small extent, ``standard'' standard errors and basic sandwich standard errors that do not take the cluster dimension into account, deteriorate with an increasing $\rho$. In addition, PCSE and Driscoll and Kraay standard errors do not perform sufficiently well because of a too small $N_g$ and because of the correlation structure chosen for the copula. With an AR(1) correlation structure, both methods perform well.
Figure \ref{fig:sim-08} shows that for a copula with AR(1) correlation, the coverage tends towards 0.95 for all regressors the higher the number of clusters \code{G}.
...
One can thus conclude that clustered standard errors will also work for glms.
...
xyplot(coverage ~ nround | factor(par) + factor(copula), groups = ~ factor(vcov),
  ...
  panel = panel.xyref)
@ 
\caption{Copula with AR(1) correlation structure: Gaussian}
\label{fig:sim-0607}
\end{figure}
...
my.settings[["strip.border"]]$col <- "black"
my.settings[["superpose.line"]]$lwd <- 1
levels(s08$vcov)[c(2,1,3)] <- c("CL-0", "PL", "PC")
s08$vcov <- factor(s08$vcov, levels(s08$vcov)[c(2,1,3)])
my.settings[["superpose.line"]]$col <- c("#377eb8","green", "#006400")
my.settings[["superpose.symbol"]]$col <- c("#377eb8","green", "#006400")
xyplot(coverage ~ nround | factor(par) + factor(dist), groups = ~ factor(vcov),
  ...
  panel = panel.xyref)
@ 
\caption{Copula with AR(1) correlation structure: Binomial, Poisson}
\label{fig:sim-08}
\end{figure}
...
s05$vcov <- factor(s05$vcov, levels(s05$vcov)[c(1:3)])
levels(s05$dist) <- c("gaussian", "binomial(logit)", "poisson")

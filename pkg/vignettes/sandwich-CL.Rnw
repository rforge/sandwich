...
GLM exponential family. To move beyond the GLM, we also consider the beta (\code{beta}, with logit link and fixed precision parameter $\phi = 10$),
...
is the outcome measure of interest. If standard errors are estimated

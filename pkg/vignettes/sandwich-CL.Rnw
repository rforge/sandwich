...
$I_{n_{g}}$ is an identity matrix of size $n_{g}$, $H$ is a block-diagonal hat matrix with clusterwise segments of the full hat matrix $H_{gg}$.
...
\fixme{Stress that not just the diagonal, as computed by hatvalues(), is needed but cluster-wise segments of the full hat matrix.

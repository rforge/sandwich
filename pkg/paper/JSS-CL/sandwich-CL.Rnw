...
clusters $G$ must approach infinity\new{\footnote{\new{Asymptotic results can also be achieved in situations when the number of clusters is fixed, while the number of observations within a cluster approaches infinity \citep{hac:Bester+Conley+Hansen:2011}. In such situations, valid inference can be conducted by adapting the t statistic by $\sqrt{G/(G-1)}$.}}}, not just the number of observations $n$

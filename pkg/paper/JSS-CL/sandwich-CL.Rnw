...
Similar to GEE, generalized least squares (e.g., available in the \pkg{nlme} package, \citealp{hac:Pinheiro+Bates+DebRoy+Sarkar:2017}) and feasible generalized least squares (e.g., available in the \pkg{plm} package, \citealp{hac:Croissant+Millo:2008}) are able to account for cluster correlations, but do affect the parameter estimator. 
A different approach in the economics panel data literature is the ``pooling model'', where the parameter estimator is not altered.
}
...
\new{
Inference with clustered covariances but also two-way and multi-way clustered covariances for more than one cluster dimension is examined in \cite{hac:Cameron+Gelbach+Miller:2011}. In a wider context, \cite{hac:Cameron+Miller:2015} discuss methods where the method of estimation is altered, instead of just adapting the inference.
\cite{hac:Cameron+Miller:2015} established a perspective on clustering where observations are correlated within clusters, but are uncorrelated across clusters, assuming that in addition to the number of
observations $n$ the number of clusters $G$ also approaches infinity.
An alternative and less frequently-used asymptotic framework to this popular
...
\new{
In a recent paper, \cite{hac:Abadie+Athey+Imbens+Wooldridge:2017} provide for a condensed summary of state-of-the-art methods for clustered covariances, focussing on situations in economic experiments where  potentially several cluster dimensions exist. In particular the sampling process as well as the assignment mechanism exhibiting a systematic variation with clusters in the sample serve as justification for using clustered covariances. However, in random experiments, clustered samples are -- with respect to clustered covariances -- less crucial than clustered assignments to various treatments. In case of a fixed effects regression with fixed effects at the cluster level, heterogeneity of the treatment effect is in fact a requirement for clustered covariances to be necessary.
}

\new{
Following \cite{hac:Cameron+Gelbach+Miller:2011}, the focus is on situations where the parameter estimator is not adapted. In accordance with the modular implementation of clustered covariances in \pkg{sandwich}, our attention is limited to adjusting inference only.  
}
...
\new{
Covariance estimators in \pkg{car} as well as in \pkg{sandwich} are constructed to insert the resulting covariance matrix estimate in different Wald-type inference functions, as \code{confint()} from \pkg{stats} package \citep{hac:R:2017}, \code{coeftest()} and \code{waldtest()} from \pkg{lmtest} package \citep{hac:Zeileis+Hothorn:2002}, \code{linearHypothesis()} and \code{deltaMethod()} from \pkg{car} package \citep{hac:Fox+Weisberg:2011}.
}
...
are subtracted. 
\new{Particularly when there is only one observation in each intersection of $id$ and $time$, \cite{hac:Petersen:2009}, \cite{hac:Thompson:2011} and \cite{hac:Ma:2014}

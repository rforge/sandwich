...
matrix $H$ that pertains to cluster~$g$. To apply these factors to
$\hat M_\mathrm{CL}$ (Equation~\ref{eq:meatCL}), their square root has to be applied
to each of the $\psi$ factors in the outer product
...
\end{equation}  
with $\alpha = 1$ for HC2 and $\alpha = 2$ for HC3, before obtaining the adjusted
...
(from 10,000 replications) is plotted on the
$y$-axis against the cluster correlation $\rho$ (from a Gaussian copula) on the $x$-axis.
The horizontal reference line indicates the nominal coverage of $0.95$.}
\label{fig:sim-03}
\end{figure}
...
\pkg{countreg} is available from \proglang{R}-Forge at \url{https://R-Forge.R-project.org/projects/countreg/}.

...
multiple objects that are correlated within, but not acress, clusters, leading
...
\fixme{Better delay the comments regarding the package until the very end of this
subsection. Instead focus on continuing the conceptual discussion, i.e., explain what
psi is before fleshing out more details.}
...
\fixme{Should the equation below get a number so that the corresponding cluster-aggregated version can refer to it.}
...
\fixme{Explain that for the HC type the model has to be of a form that allows factorization of the scores psi
into a working residual times regressor vector. Preferably, the clustered HCs would then also connect to this.}
...
\fixme{Explain the switch to the notation with double index.}
...
\readme{I changed the notation here to explicitly show the some, hoping this is easier to understand. Also, this should probably have
number to be able to refer back and forth between the unaggregated and cluster-aggregated version of this matrix.}
 \[ \left( \begin{array}{c} \psi(y_{11}, x_{1}, \hat \theta) + \dots + \psi(y_{n_1 1}, x_{n_1 1}, \hat \theta)\\
           \vdots \\
	   \psi(y_{1G}, x_{1G}, \hat \theta) + \dots + \psi(y_{n_G 1}, x_{n_G 1}, \hat \theta)
           \end{array} \right), \]
...
\fixme{Rather than calling it `White' refer to Equation~6 where also a more neutral term should be adopted.}
...
\fixme{try to convey at which point the HC adjustment needs to be inserted in Equation~9}
This is an easy task for HC0 and HC1 types of bias correction, but more demanding for cluster generalizations of HC2 and HC3, which have been developed by \cite{hac:Kauermann+Carroll:2001} and \cite{hac:Bell+McCaffrey:2002}.
...
\end{eqnarray}  
\fixme{be consistent about bolding or not bolding matrix names}
...
\fixme{When put together an overall H would be block-diagnoal but the individual $H_{gg}$ is not, is it?}

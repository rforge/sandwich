...
\end{equation}  
into empirical working residuals times the regressor vector. 
...
$I_{n_{g}}$ is an identity matrix of size $n_{g}$, $H$ is a block-diagonal hat matrix with cluster-wise segments of the full hat matrix $H_{gg}$.
...
\cite{hac:Petersen:2009}, \cite{hac:Thompson:2011} and \cite{hac:Ma:2014} suggest to substract the standard sandwich estimator  in case that the clusters formed by the intersection of $id$ and $time$ do only contain a single observation\footnote{If one wants to cluster by dimensions id and year in a sample of id-month observations, each cluster is composed of 12 observations \citep{hac:Ma:2014}. In such a setting, the last substracted matrix should be a one-way clustered covariance matrix with clusters formed by the intersections of id and year.}. 
...
The cluster dimension is now $t$ with $t = 1, \ldots, T$ time periods and $n_t$ observations in a time period.
...
\end{equation}  
where $w_{|i-j|} = 1 - \frac{|i-j|}{L + 1}$ is a distance-decreasing kernel function with a lag $\ell = |i-j|$, $L$ is the maximal lag length. As \cite{hac:Petersen:2009} noticed, the maximal lag length in a panel data set is the maximum number of time periods per id minus one.
%\fixme{Avoid jargon with `years' and `firms'. Employ these only as illustrations in the beginning and then stick to `time' and `id' instead.}
...
%\fixme{Is this paragraph necessary? It's currently `hanging' at the end of the subsection without a proper
%conclusion. Also, the notion of `residuals' is linear-model-specific and should ideally be avoided.}
...
\begin{equation} \label{eq:driscoll}
  \hat M_\mathrm{PL}^{DC} \quad = \quad \frac{1}{n} \sum_{t = 1}^T \sum_{i, j = 1}^{n_t} w_{|i-j|} \, \psi(y_{it}, x_{it}, \hat \theta) \psi(y_{jt}, x_{jt}, \hat \theta)^\top,
\end{equation}  
...
%\fixme{Emphasize that this is really combining the clustered idea, already introduced in the previous subsection, %and the HAC idea.
%And then adapt the notation as above.}

\subsection{Panel-corrected standard errors}
%\fixme{The start of this subsection conveys the impression that this is very different from the ideas above:
%The new TSCS abbreviation is introduced and some `Parks-Kmenta' method is mentioned. This can be quite
%confusing. Instead focus from the beginning on what is similar to above and what is different.}
...
%\fixme{The working residuals need to be introduced above so that they can be reused several times through this %entire section.}
...
\begin{tabular}{llll}
  ...
  \code{PC} & \code{lm} & \code{m} & \code{vcovPC(m, cluster = id,
                           order.by = round, kronecker = TRUE)} \\ \hline

...
Standard errors being compared to each other include iid standard errors without heteroscedasticity and without autocorrelation (classical), HC0 standard errors \citep{hac:White:1980}, Driscoll and Kraay standard errors (DK) \citep{hac:Driscoll+Kraay:1998}, panel-corrected (PC) standard errors a la \cite{hac:Beck+Katz:1995} and clustered standard errors with HC0 to HC3 adjustment (HC0-cluster, HC1-cluster, HC2-cluster, HC3-cluster). In addition, standard errors from a random effects model (random) and from a GEE with exchangeable correlation structure (gee) are compared.
...
my.settings[["superpose.line"]]$col <- c("#006400", "#0000FF", "#e41a1c", "#f781bf", "#377eb8", "#984ea3", "#ff7f00")
my.settings[["superpose.symbol"]]$col <- c("#006400", "#0000FF", "#e41a1c", "#f781bf", "#377eb8", "#984ea3", "#ff7f00")
...
The packages \pkg{sandwich}, \pkg{pscl}, \pkg{lattice}, \pkg{countreg}, \pkg{lmtest}, \pkg{plm}, \pkg{geepack}, \pkg{multiwayvcov} and \pkg{lme4} are
...
and \pkg{lme4} \Sexpr{gsub("-", "--", packageDescription("lme4")$Version)}
have been used.
...
$\rho$ determines again the importance of the random cluster effect and ranges from 0 to 0.9.

Contrasting coverage and bias plots for models except the linear model depicts the problem caused by this approach.

\begin{figure}[thb] \label{fig:sim-05-01}
  \begin{center}
...
Figure \ref{fig:sim-05-01} constrasts coverage and bias plots for Gaussian, Binomial and Poisson response distributions with a single regressor $x_{1,ig}$. Each of the 10,000 simulated datasets consists of 100 clusters with 5 observations per cluster.
For the Gaussian response, the random cluster effect does not introduce a bias, but for the Binomial and Logit responses, this happend. The higher the importance of the random cluster effect $\rho$, the worse the bias.
...
Introducing within cluster correlation via a copula as in \ref{fig:sim-02-1}, unbiased estimators for the response distribution occur and thus also do not have an impact on the coverage.

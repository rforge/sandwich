...
extracted from the model object. Model classes for which an \code{estfun} method is available currently
include \code{betareg}, \code{clm}, \code{coxph}, \code{crch}, \code{glm}, \code{hurdle}, \code{lm}, \code{mlm}, \code{mlogit},

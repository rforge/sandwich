...
$w_{|i-j|}$ in (\ref{eq:meat-hac}) is a vector of weights \citep{hac:Zeileis:2004a}
...
One-, two-, and multi-way clustered standard errors with the different types of bias correction introduced in (\ref{eq:biasadj1})--(\ref{eq:biasadj4}) are implemented with the following arguments in the function
\begin{verbatim}
...
The bias correction is composed of two parts, the cluster bias correction defined in (\ref{eq:biasadj0}) and the HC bias correction defined in  (\ref{eq:biasadj1})--(\ref{eq:biasadj4}). Both types of bias correction can be separately switched on and off.
With the \code{cadjust} argument, the cluster bias adjustment can be turned on and off, whereas the \code{type} argument allows to specify the HC type of bias correction.
...
vc <- list(
  ...
  "plm" = vcovHC(p_plm, type = "HC0", cluster = "group") * bc,
  "geepack" = vcov(p_gee) * bc
)
...
\subsection[Aghion et al. (2013) and Berger et al. (2016)]{\cite{hac:Aghion+VanReenen+Zingales:2013} and \cite{hac:Berger+Stocker+Zeileis:2016}} \label{ex-aghion}
...
Therefore, \cite{hac:Berger+Stocker+Zeileis:2016} employ two-part hurdle models with a binary part that models the decision to innovate at all, and a count part that models ongoing innovation, respectively.
...
Standard errors being compared to each other include iid standard errors without heteroscedasticity and without autocorrelation (\emph{classical}), HC0 standard errors \citep{hac:White:1980}, Driscoll and Kraay standard errors (\emph{DK}) \citep{hac:Driscoll+Kraay:1998}, panel-corrected standard errors (\emph{PC}) a la \cite{hac:Beck+Katz:1995} and clustered standard errors with HC0 to HC3 adjustment (\emph{HC0-cluster}, \emph{HC1-cluster}, \emph{HC2-cluster}, \emph{HC3-cluster}). In addition, standard errors from a random effects model (\emph{random}) and from a GEE with exchangeable correlation structure (\emph{gee}) are compared.
...
\end{itemize}
Figure~\ref{fig:sim-01} shows the results from Experiment I and plots the coverage probabilities for the coefficients $x1$ (\ref{x1}), $x2$ (\ref{x2}) and $x3$ (\ref{x3}) on the y-axis. Regressor $x1$ is composed of a combination of a random draw at cluster level and a random draw at individual level, with a cluster correlation of 0.25. Regressor $x2$ consists of a random draw at cluster level, and regressor $x3$ is composed of a random draw at individual level. 
...
Starting with regressor $x1$, at $\rho = 0$, all standard errors except the Driscoll and Kraay standard errors perform well. It can be observed that when the number of observations within each cluster is small with only $N_g = 5$, Driscoll and Kraay standard errors are substantially underestimated, even for a $\rho$ of 0.
Panel-corrected standard errors for regressor $x1$ underestimate the ``true'' standard errors, that applies all the more the higher $\rho$. To a smaller extend, the same holds true for ``classical'' standard errors.
...
Figure~\ref{fig:sim-02} illustrates the results from Experiment II. The y-axis represents again the coverage, and $\rho$ is plotted on the x-axis. Three response distributions are compared: Gaussian, binomial (with a logit link) and Poisson. Unlike in Experiment I, only a single regressor $x1$ is included, with a $\rho_x$ of 0.25.
...
It can be observed that for regressor $x2$, Driscoll and Kraay standard errors, PCSE, and to a smaller extend also ``classical'' and HC0 standard errors are getting worse the larger the within cluster correlation.
...
Figure~\ref{fig:sim-02} illustrates the results from Experiment II. The y-axis represents again the coverage, and $\rho$ is plotted on the x-axis. Three response distributions are compared: Gaussian, binomial (with a logit link) and Poisson distribution. Unlike in Experiment I, only a single regressor $x1$ is included, with a $\rho_x$ of 0.25.

Figure~\ref{fig:sim-03} shows the outcome of Experiment III.

Figure~\ref{fig:sim-04} depicts the findings of Experiment IV.
...
The linear predictor in (\ref{eq:predictor}) with a random cluster effect $v_g$ is
...
Figure~\ref{fig:sim-05-01} constrasts coverage (in the upper panel) and bias (in the lower panel) for Gaussian, binomial (with a logit link) and Poisson response distributions with a single regressor $x_{1,ig}$. Regressor $x_{ig}$ exhibits a cluster correlation $\rho_x$ of 0.25. Each of the 10,000 simulated datasets consists of 100 clusters with 5 observations per cluster.

...
the \code{estfun()} function, for example, and is not defined for some nonlinear models). 
...
assumptions--to be useful in practice, even for linear models applied to large samples. The 
\code{lm} method provides several additional bootstrapping \code{type}sand computes the 
...
case-based resampling (\code{type = "xy"}) as in the default method. Alternative \code{type} 
specifications are
%
\begin{itemize}
  \item

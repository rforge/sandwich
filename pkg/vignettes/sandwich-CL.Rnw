...
\new{
effectively assuming that each cross-sectional group is a separate, autocorrelated time-series
}%
. The meat is composed of
\new{
...
Because the ``xy'' method makes the same core assumptions as the asymptotic approach above, its results approach
the asymptotic results as the number of bootstrap replications $R$ becomes large. This makes ``xy'' bootstraps
unnecessary if the asymptotic estimator is available (this may not be the case if a software package has not implemented
the \code{estfun()} function, for example). However, the literature has found a number of alternative bootstrapping
approaches, which make somewhat different assumptions, to be useful in practice.
The \code{lm} method provides several additional bootstrapping \code{type}s and computes the

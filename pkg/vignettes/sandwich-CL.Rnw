...
By observing this, we took a closer look at panel covariances in case of (1) ``longer'' panels, and (2) an AR(1) structure instead of on exchangeable correlation structure.
...
For the uncorrelated regressor \code{x3}, panel-corrected covariances perform well even for $5$ observations per cluster, but underestimation is getting even more pronounced the higher the cluster correlation of a regressor.

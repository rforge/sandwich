...
\new{According to \cite{hac:Abadie+Athey+Imbens+Wooldridge:2017}, clustered covariances are conservative in case of heterogeneity in treatment effects and if the number of clusters (as well as the number of observations within each cluster) observed is large enough.}
...
\new{In case of a fixed effects regression (with cluster level fixed effects) clustered covariances are necessary only when treatment heterogeneity is present \citep{hac:Abadie+Athey+Imbens+Wooldridge:2017}.}
...
\subsection{Clustered covariances in Stata}

\new{
Official Stata as well as the Stata community provide implementations for several types of clustered covariances.
One-way clustered covariances are available in Official Stata for a wide range of estimators (``ivreg2'', ``xtreg'', ``reg'', among others) by adding the ``cluster'' option, and additionally supports cluster correction via the ``suest'' command for a system of equations. In the presence of nested multi-level clustering, the ``syv'' suite of commands can be employed \citep{hac:Nichols+Schaffer:2007}. Panel-corrected standard errors are supported by ``xtpcse''. 
The Stata Community package ``avar'' \citep{hac:Nichols+Schaffer:2014} allows to construct the meat matrix of various flavors of sandwich covariance estimators, including among others HAC, one- and two-way clustering, Driscoll and Kraay covariances as well as Kiefer covariances \citep{hac:Kiefer:1980}.
The Stata add-on ``xtscc'' provides a further implementation of Driscoll and Kraay covariances \citep{hac:Hoechle:2007}.}
...
\new{In the statistics literature, the basic sandwich estimator \citep{hac:Eicker:1963,hac:Huber:1967,hac:White:1980} has been extended for clustered units by \cite{hac:Kloek:1981} and \cite{hac:Moulton:1990}, using a correlation structure equivalent to the exchangeable correlation in the GEE framework as introduced by \cite{hac:Liang+Zeger:1986}.
\cite{hac:Kiefer:1980} has implemented yet another clustered covariance estimator, which is robust to clustering but assumes homoscedasticity \citep[see also][]{hac:Baum+Nichols+Schaffer:2011}.
The structure of equal correlation whithin clusters imposed by exchangeable correlation has been generalized further within the GEE framework, employing an independence working model \citep{hac:Liang+Zeger:1986}.

...
Theory based on a CLT, estimating functions

Implementation assumes that model fitters are available from which the bread
can directly be derived. For the meat only the estimating functions need to be derived
from which various types of meat can be computed.
